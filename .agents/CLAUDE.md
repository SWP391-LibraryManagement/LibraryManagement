...
- **Post-release application baseline**: FE10 v0.5.0 governance PR #70 is merged as `25c09ec`; current upstream baseline is `main@a240705`. PR #75 head `28c4f80` passed exact-head CI and Azure staging historically. The H1-approved H3 remediation branch is now rebased to committed head `8831571`; its remediation diff remains uncommitted because H2 round 3 was superseded before use by this later drift. Published `v1.0.2` remains at `c988af1`. A future release tag requires review of later batches, current dependency audit, and explicit human release approval.
...
- **Current SDD scope**: FE01-FE12 baseline documentation is **APPROVED** by Nhat on 2026-07-17. Phase 2 implementation, reconciliation, validation, human acceptance, merge, and exact post-merge CI are complete for that approved scope. FE10 v0.5.0 personal-inbox design, written SPEC, implementation plan/H1, earlier drift addenda, implementation H2, PR CI, Azure migration/deployment, and live three-role verification are complete through PR #75 head `28c4f80`. H1 drift reconciliation through `main@a240705` is approved and applied locally while preserving the upstream FE11 edit-action removal; bounded H3 ADR/documentation/UI remediation now requires fresh H2/CI/Azure/H3 before merge.
  ...
  - FE10 historical G1-G7, ADR-004/G8-G10 OTP delivery, FE11 `ACCOUNT_SETUP`, and G12 FE04 membership-result integration are complete through their approved slices; real provider delivery was observed PASS in live run `c6e0c46421f0`. The v0.5.0 personal inbox is implemented through FE10-I08 on PR #75 head `28c4f80`; H2, exact-head CI `30306805399`, Azure staging `30307855616`, and live verification passed, but H3 remediation, fresh gates, merge, and exact post-merge CI/deployment remain; FE09 caller integration remains deferred.
...
- **Phase 2 exit state**: the prior approved FE01-FE12 baseline completed Phase 2 with 100% enforced traceability; the canonical historical evidence is `.sdd/reviews/phase2-full-exit-validation-2026-07-19.md`. FE10 v0.5.0 retains historical Azure proof for PR head `28c4f80`; the latest automatic staging deployment is current `main@a240705` via run `30311973740`, so the rebased remediation candidate must be committed after H2 and redeployed at its new exact head before repeated H3.
