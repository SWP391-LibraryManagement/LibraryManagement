...
- **Post-release application baseline**: FE10 v0.5.0 governance PR #70 is merged as `25c09ec`; current upstream baseline is H1-approved `main@30f936d`. PR #75 historical head `cf0a236` passed exact-head CI `30315046007`, Azure staging `30315273298`, and two-axis H3 with no actionable finding. The branch is rebased locally as `c72bc77` while preserving the upstream Vietnamese SDD translation; fresh H2, exact-head CI/Azure, repeated H3, merge, and post-merge gates remain mandatory. Published `v1.0.2` remains at `c988af1`. A future release tag requires review of later batches, current dependency audit, and explicit human release approval.
...
- **Current SDD scope**: FE01-FE12 baseline documentation is **APPROVED** by Nhat on 2026-07-17. Phase 2 implementation, reconciliation, validation, human acceptance, merge, and exact post-merge CI are complete for that approved scope. FE10 v0.5.0 personal-inbox design, written SPEC, implementation plan/H1, implementation/remediation, Azure migration/deployment, bounded live verification, and a clean H3 review are complete on the pre-translation lineage through `cf0a236`. The approved `main@30f936d` documentation drift requires a new H2 and exact-head integration evidence before final H3/merge.
  ...
  - FE10 historical G1-G7, ADR-004/G8-G10 OTP delivery, FE11 `ACCOUNT_SETUP`, and G12 FE04 membership-result integration are complete through their approved slices; real provider delivery was observed PASS in live run `c6e0c46421f0`. The v0.5.0 personal inbox and bounded H3 remediation are implemented; exact-head CI `30315046007`, Azure staging `30315273298`, and final two-axis H3 passed on `cf0a236`. The approved `main@30f936d` rebase requires refreshed H2/integration gates; FE09 caller integration remains deferred.
