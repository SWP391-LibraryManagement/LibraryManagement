...
- **Post-release application baseline**: FE10 v0.5.0 governance PR #70 is merged as `25c09ec`; current upstream baseline is `main@a240705`. PR #75 runtime remediation head `3f9f23a` has H2 round 4 approval, passing CI `30313721511`, and passing Azure staging deployment `30313949983`. A bounded evidence-only lifecycle closeout must receive H2 and make the resulting latest PR head pass exact-head gates before repeated H3 and merge. Published `v1.0.2` remains at `c988af1`. A future release tag requires review of later batches, current dependency audit, and explicit human release approval.
...
- **Current SDD scope**: FE01-FE12 baseline documentation is **APPROVED** by Nhat on 2026-07-17. Phase 2 implementation, reconciliation, validation, human acceptance, merge, and exact post-merge CI are complete for that approved scope. FE10 v0.5.0 personal-inbox design, written SPEC, implementation plan/H1, drift addenda, implementation/remediation H2, exact-head PR CI, Azure migration/deployment, and bounded live verification are complete through runtime remediation head `3f9f23a`. The evidence-only lifecycle closeout, latest-head gates, repeated H3, explicit H3 approval, merge, and post-merge CI/deployment remain pending.
  ...
  - FE10 historical G1-G7, ADR-004/G8-G10 OTP delivery, FE11 `ACCOUNT_SETUP`, and G12 FE04 membership-result integration are complete through their approved slices; real provider delivery was observed PASS in live run `c6e0c46421f0`. The v0.5.0 personal inbox and bounded H3 remediation are implemented through runtime head `3f9f23a`; exact-head CI `30313721511` and Azure staging `30313949983` passed. Evidence-only closeout, latest-head gates, repeated H3, merge, and exact post-merge CI/deployment remain; FE09 caller integration remains deferred.
...
- **Phase 2 exit state**: the prior approved FE01-FE12 baseline completed Phase 2 with 100% enforced traceability; the canonical historical evidence is `.sdd/reviews/phase2-full-exit-validation-2026-07-19.md`. FE10 v0.5.0 retains historical three-role Azure proof for runtime head `28c4f80`; runtime remediation head `3f9f23a` passed CI `30313721511` and Azure deployment `30313949983`. The bounded evidence-only closeout must be published and the resulting latest PR head must pass exact-head gates before repeated H3.
