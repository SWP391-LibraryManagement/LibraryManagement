...
- **Post-release application baseline**: FE10 v0.5.0 governance PR #70 is merged as `25c09ec`; current implementation baseline is upstream `main@f3ebe95` after the approved CI-gated deployment/Core-drift addenda and a non-overlapping test-only sync. Published `v1.0.2` remains at `c988af1`. A future release tag requires review of later batches, current dependency audit, and explicit human release approval.
...
- **Phase 2 exit state**: the prior approved FE01-FE12 baseline completed Phase 2 with 100% enforced traceability; the canonical historical evidence is `.sdd/reviews/phase2-full-exit-validation-2026-07-19.md`. FE10 v0.5.0 now truthfully declares a post-drift locally validated candidate awaiting fresh H2; staging, integration, and post-merge evidence are not claimed early.
