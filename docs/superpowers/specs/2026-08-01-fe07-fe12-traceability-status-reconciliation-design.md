# FE07-Thiết kế đối chiếu trạng thái truy vết FE12
...
- Bổ sung kiểm tra hồi quy đọc trực tiếp bốn `SPEC.md` và từ chối mọi hàng ma
  trận có trạng thái khác `Hoàn thành` khi bốn chức năng đang là `COMPLETE`.

## Ngoài phạm vi

- Không đổi trạng thái vòng đời nghiệp vụ như `PENDING`, `ACTIVE`, `NOTIFIED`
  hoặc `BORROWED`.
- Không đổi mã nguồn giao diện, máy chủ, API, cơ sở dữ liệu, quyền hoặc quy tắc
  nghiệp vụ.
- Không viết lại nội dung bằng chứng hay bổ sung tuyên bố hoàn thành mới.

## Phương án

Chọn kiểm tra hồi quy trong `scripts/traceability-state.test.js` vì tệp này
đã sở hữu quy tắc `Implementation State: COMPLETE` và kiểm tra trạng thái phát
hành của đúng bốn chức năng. Kiểm tra chỉ phân tích các hàng Markdown nằm trong
...
2. Sau khi sửa, mọi hàng yêu cầu trong Phần 16 của bốn chức năng có cột cuối là
   `Hoàn thành`.
