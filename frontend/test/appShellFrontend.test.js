...
test('admin console keeps its sections while using the warm librarian visual system', async () => {
  ...
  assert.match(source, /borrowingApi\.approve\(viewRequest\.id\)/);
  assert.match(source, /borrowingApi\.reject\(viewRequest\.id, reason\)/);
  ...
  assert.match(source, /setActiveSection\('circulation'\)/);
