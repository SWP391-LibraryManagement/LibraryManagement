    ...
    <>
      <Modal title={book.title} eyebrow="FE06 • Bản sao vật lý" onClose={onClose} width={960}>
        <DataTable
          ...
          headers={['ID', 'Barcode', 'Vị trí', 'Trạng thái', { label: 'Thao tác', align: 'right' }]}
              ...
              <tr key={copy.copyId}>
                ...
                <td data-label="Thao tác" style={{ textAlign: 'right' }}>
                  <div className="row-flex" style={{ justifyContent: 'flex-end' }}>
                    ...
                    <button type="button" className="btn btn-danger btn-sm" onClick={() => setDeactivateTarget(copy)} disabled={saving || copy.status === 'INACTIVE'}><DeleteIcon fontSize="small" /> Ngừng</button>
                  </div>
                </td>
              </tr>
            );
        ...
        <DataToolbar
          ...
          filters={(
            <>
              <input className="input" value={newCopy.location} onChange={(event) => setNewCopy({ ...newCopy, location: event.target.value })} placeholder="Vị trí" aria-label="Vị trí bản sao mới" />
              <select className="select" value={newCopy.status} onChange={(event) => setNewCopy({ ...newCopy, status: event.target.value })} aria-label="Trạng thái bản sao mới">
