...
export default function BorrowingHistoryPage() {
  const [rows, setRows] = useState(DEMO_BORROW_ROWS);
  ...
  async function confirmRenew() {
    try {
      if (!isDemo) {
        ...
        setRows((prev) => prev.map((row) => row.id === renewRow.id ? { ...row, dueDate: addDays(row.dueDate, 14), renewalsLeft: 0 } : row));
      }
