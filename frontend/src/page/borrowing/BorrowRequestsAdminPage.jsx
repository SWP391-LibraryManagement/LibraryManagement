...
export default function BorrowRequestsAdminPage() {
  ...
  return (
    <AppLayout active="borrow-requests-admin" title="Yêu cầu mượn sách" subtitle="Thủ thư duyệt hoặc từ chối yêu cầu; hệ thống kiểm tra lại điều kiện mượn trước khi xác nhận." actions={<button className="btn btn-outline" onClick={loadRequests} disabled={loading}><RefreshCw size={16} /> Tải lại</button>}>
      {notice && <DataNotice type="error" title="Không thể tải yêu cầu mượn">{notice}</DataNotice>}
      {loading ? <LoadingBlock rows={4} /> : <div className="split borrow-request-split">
        <div className="lib-table-wrap"><table className="lib-table"><caption className="sr-only">Borrow requests table</caption><thead><tr><th scope="col">Yêu cầu</th><th scope="col">Thành viên</th><th scope="col">Sách</th><th scope="col">Ngày gửi</th><th scope="col">Trạng thái</th></tr></thead><tbody>{requests.map((row) => <tr key={row.id} tabIndex={0} aria-label={`Select borrow request ${row.id}`} onClick={() => setSelectedId(row.id)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setSelectedId(row.id); } }} style={{ cursor: 'pointer', background: row.id === selectedId ? 'var(--lib-accent-bg)' : undefined }}><td><strong>{row.id}</strong></td><td>{row.member}</td><td>{row.book}</td><td>{fmtDate(row.requestDate)}</td><td><Badge status={row.status} /></td></tr>)}</tbody></table>{requests.length === 0 && <EmptyState icon={ClipboardList} title="Không có yêu cầu nào" />}</div>
        <div className="panel">{selected ? <><div className="panel-header"><div className="app-avatar">{String(selected.member).slice(0, 1)}</div><div className="stack-sm"><strong>{selected.member}</strong><span className="muted" style={{ fontSize: 13 }}>{selected.id} • {fmtDate(selected.requestDate)}</span></div><span style={{ marginLeft: 'auto' }}><Badge status={selected.status} /></span></div><div className="info-list"><div className="info-row"><Hash size={16} /><span className="muted">Mã TV:</span> <strong>{selected.memberId}</strong></div><div className="info-row"><Mail size={16} /> {selected.email}</div><div className="info-row"><Phone size={16} /> {selected.phone}</div></div><h4 className="section-title">Sách yêu cầu</h4><div className="row-flex" style={{ alignItems: 'flex-start', gap: 14 }}><span className="book-spine" style={{ background: 'linear-gradient(135deg,#a87532,#7b5528)', width: 36, height: 50 }} /><div className="stack-sm"><strong>{selected.book}</strong><span className="muted" style={{ fontSize: 13 }}>{selected.author}</span><span className="muted" style={{ fontSize: 13 }}>Copy #{selected.copyId} • {selected.branch}</span><span className={`badge badge-${selected.copyAvailable ? 'available' : 'overdue'}`} style={{ width: 'fit-content', marginTop: 4 }}>{selected.copyAvailable ? 'Bản sao sẵn sàng' : 'Cần kiểm tra lại tình trạng bản sao'}</span></div></div>{selected.status === 'Pending' && <div className="modal-actions" style={{ borderTop: '1px solid var(--lib-line)', marginTop: 20, paddingTop: 16, paddingLeft: 0, paddingRight: 0 }}><button className="btn btn-danger" onClick={() => setRejecting(true)}><ThumbsDown size={16} /> Từ chối</button><button className="btn btn-primary" onClick={() => setApproveTarget(selected)}><ThumbsUp size={16} /> Duyệt</button></div>}</> : <EmptyState icon={ClipboardList} title="Chọn một yêu cầu để xem chi tiết" />}</div>
      </div>}
...
function ApproveModal({ req, onClose, onApprove }) {
  return <Modal eyebrow="UC35 • Duyệt yêu cầu mượn" title={`Duyệt yêu cầu ${req.id}`} onClose={onClose} actions={<><button className="btn btn-ghost" onClick={onClose}>Hủy</button><button className="btn btn-primary" onClick={onApprove}><ThumbsUp size={16} /> Duyệt & cấp sách</button></>}><div className="info-list" style={{ marginBottom: 18 }}><div className="info-row"><span className="muted">Thành viên:</span> <strong>{req.member}</strong></div><div className="info-row"><span className="muted">Sách:</span> <strong>{req.book}</strong> (Copy #{req.copyId})</div><div className="info-row"><span className="muted">Thời gian:</span> <strong>{fmtDate(req.borrowDate)} → {fmtDate(req.dueDate)}</strong></div></div><div className={`alert-box ${req.copyAvailable ? 'info' : 'warn'}`}>{req.copyAvailable ? 'Bản sao đang được ghi nhận là sẵn sàng. Hệ thống sẽ kiểm tra lại toàn bộ điều kiện khi duyệt.' : 'Bản sao chưa được ghi nhận là sẵn sàng. Có thể thử duyệt để nhận trạng thái mới nhất từ hệ thống.'}</div></Modal>;
}
