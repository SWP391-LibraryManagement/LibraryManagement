...
export default function MemberBorrowingDetailsPage() {
  ...
  return (
    <AppLayout active="member-details" title="Chi tiết mượn sách của thành viên" subtitle="Tra cứu thông tin mượn/trả của một thành viên.">
      ...
      {loading ? (
        <DataTable caption="Loading member borrowings" headers={['Thông tin']} loading />
      ) : memberView ? (
        <>
...
function PendingTable({ rows }) {
  return (
    <div className="lib-card">
      <h3 className="lib-card-title">Yêu cầu đang chờ</h3>
      <DataTable
        caption="Yêu cầu đang chờ"
        headers={['Sách', 'Ngày yêu cầu', 'Trạng thái']}
        isEmpty={rows.length === 0}
        emptyState={<EmptyState icon={BookOpen} title="Không có yêu cầu đang chờ" />}
      >
        {rows.map((row, index) => (
          <tr key={`${row.book}-${index}`}>
            <td data-label="Sách"><strong>{row.book}</strong></td>
            <td data-label="Ngày yêu cầu">{fmtDate(row.borrowDate)}</td>
...
function LoanTable({ title, rows, emptyIcon, empty, history = false }) {
  return (
    <div className="lib-card">
      ...
      <DataTable
        caption={title}
        headers={['Sách', 'Ngày mượn', history ? 'Ngày trả' : 'Hạn trả', 'Trạng thái']}
        isEmpty={rows.length === 0}
        emptyState={<EmptyState icon={emptyIcon} title={empty} />}
      >
        {rows.map((row, index) => (
          <tr key={`${row.book}-${index}`} className={row.status === 'Overdue' ? 'row-overdue' : ''}>
            <td data-label="Sách"><strong>{row.book}</strong></td>
            <td data-label="Ngày mượn">{fmtDate(row.borrowDate)}</td>
            <td data-label={history ? 'Ngày trả' : 'Hạn trả'}>{fmtDate(history ? row.returnDate : row.dueDate)}</td>
            <td data-label="Trạng thái"><Badge status={row.status} /></td>
          </tr>
        ))}
      </DataTable>
