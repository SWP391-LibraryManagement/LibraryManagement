      ...
      {loading ? <LoadingBlock rows={4} /> : view === 'list' ? (
        <>
          <div className="toolbar">
            ...
            <select className="select" style={{ width: 200 }} value={bookFilter} onChange={(e) => { setBookFilter(e.target.value); setPage(1); }}>{books.map((book) => <option key={book} value={book}>{book === 'ALL' ? 'Tất cả sách' : book}</option>)}</select>
            <select className="select" style={{ width: 190 }} value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setPage(1); }}>{STATUSES.map((status) => <option key={status} value={status}>{status === 'ALL' ? 'Tất cả trạng thái' : status}</option>)}</select>
          </div>
          <div className="lib-table-wrap">
            <table className="lib-table"><thead><tr><th>Thành viên</th><th>Sách</th><th>Ngày đặt</th><th>Vị trí</th><th>Trạng thái</th></tr></thead><tbody>{pageRows.map((item) => <tr key={item.id}><td>{item.member}</td><td><strong>{item.book}</strong></td><td>{fmtDate(item.reservedDate)}</td><td>#{item.queue}</td><td><Badge status={item.status} /></td></tr>)}</tbody></table>
            {pageRows.length === 0 && <EmptyState icon={CalendarClock} title="Không có đặt chỗ phù hợp" />}
          </div>
          <div className="pagination"><span className="muted">{filtered.length} đặt chỗ • trang {safePage}/{totalPages}</span><div className="page-controls"><button className="page-btn" disabled={safePage <= 1} onClick={() => setPage(safePage - 1)}><ChevronLeft size={16} /></button>{Array.from({ length: totalPages }, (_, i) => <button key={i} className={`page-btn${safePage === i + 1 ? ' active' : ''}`} onClick={() => setPage(i + 1)}>{i + 1}</button>)}<button className="page-btn" disabled={safePage >= totalPages} onClick={() => setPage(safePage + 1)}><ChevronRight size={16} /></button></div></div>
        </>
      ) : (
        <div className="lib-card">
          <div className="toolbar">
            <h3 className="lib-card-title" style={{ margin: 0 }}>Hàng đợi chờ</h3>
            <select className="select" style={{ width: 220 }} value={queueBook} onChange={(e) => setQueueBook(e.target.value)}>{queueBooks.map((book) => <option key={book} value={book}>{book}</option>)}</select>
