...
mẫu, lưu vào cơ sở dữ liệu an toàn, khả năng xử lý lặp an toàn, xử lý xếp hàng, thử lại và hành vi hộp thư
...
- Quy tắc dữ liệu an toàn/che dữ liệu.
...
- Không lưu vào cơ sở dữ liệu, ghi log, ghi kiểm toán hoặc phản hồi OTP/liên kết thiết lập/
  nội dung nhạy cảm đã tạo từ mẫu.

## 3. Mục tiêu kiểm thử API / tích hợp

- `POST /notifications/requests`: luồng thành công, mẫu không hợp lệ, thiếu
  người nhận, bị cấm.
- Thành phần gửi yêu cầu FE02 trong tiến trình: thành công/thất bại OTP xác minh/đặt lại,
  biến, khả năng xử lý lặp an toàn và không gửi trùng.
- Thành phần gửi yêu cầu FE11 trong tiến trình: thành công/thất bại thiết lập tài khoản,
...
- Cổng bao phủ backend đầy đủ sau cập nhật nhánh `main@12faead` đã được phê duyệt đạt
...
- Bằng chứng frontend FE10-I04..I06 đạt 259/259 kiểm thử Node, ESLint và bản
  dựng môi trường triển khai thực tế Vite. Nó bao phủ bốn phương thức phía máy khách được ủy quyền, làm mới
...
- Kiểm toán ranh giới H2 cuối thay đổi liên kết SQL `@Offset` từ `INT` thành
  `BIGINT`; kiểm thử hồi quy tầng truy cập dữ liệu đạt 5/5 và SQL Server chấp nhận probe
...
- Kiểm tra bảo mật mới sau `main@12faead` xác nhận không có lỗ hổng trong thư
  viện phụ thuộc của backend, bí mật có độ tin cậy cao mới được thêm, dấu xung
  đột hoặc mâu thuẫn hộp thư
  bị trì hoãn lỗi thời hoặc trường phản hồi không an toàn. Cổng kiểm toán
  frontend tầng truy cập dữ liệu chỉ chấp nhận GHSA-qwww-vcr4-c8h2 vì ứng dụng dùng
  `BrowserRouter` khai báo và không dùng API RSC không ổn định được cảnh báo đề
  cập.
- Lượt đánh giá dừng an toàn cuối cùng bổ sung kiểm thử hồi quy trước và sau khi
  sửa, yêu cầu cả hai câu truy vấn SQL lấy hàng chờ phải loại trừ `ACCOUNT_SETUP`
  cùng các loại nhạy cảm khác. Trước khi sửa, kiểm thử chỉ thất bại với hàm hỗ
  trợ `listPending` cũ; sau khi sửa, 3 bộ/161 kiểm thử tập trung đều đạt và toàn
...
- Việc cập nhật nhánh lên `main@a240705` đã được phê duyệt và giữ nguyên việc loại bỏ thao tác chỉnh
  ...
  trạng thái truy vết 3/3, truy vết FE10 14/16 (88%), Chromium 11/11,
  kiểm toán thư viện phụ thuộc, chuẩn bị cấu trúc cơ sở dữ liệu Azure và vệ sinh
  phần thay đổi. Số lượng backend
  giảm chỉ vì phần triển khai trước đã loại bỏ các kiểm thử chỉnh sửa FE11 lỗi thời.
...
- `backend/tests/notificationRoutes.test.js` bao phủ quyền sở hữu chuẩn, gửi
  ...
  phản hồi an toàn, khả năng xử lý lặp an toàn, xếp hàng, xử lý, thử lại và lỗi xác thực.
- `backend/tests/integration.test.js` bao gồm bằng chứng tích hợp hộp thư cá nhân
  FE04/FE07/FE08.
- `backend/tests/fe10OtpTemplateMigration.test.js`
- Truy vết: bao phủ `@spec` FR **100%** (`npm run trace:enforce`).
...
- Bằng chứng quyền sở hữu mở rộng bao phủ cả hai loại nhạy cảm FE02 đối với mọi
  thành phần gửi yêu cầu trong danh sách cho phép khác, lỗi ghi đè nguồn HTTP chính xác
  và xoay vòng sự kiện mã thông báo đặt lại lặp lại.

## 6. Khoảng trống

- `database/Librarymanagement.sql`, các độ rộng dùng chung do FE11 sở hữu và
  bản cập nhật cơ sở dữ liệu OTP FE10 đã được đồng bộ; bản cập nhật cơ sở dữ liệu đã qua hai lần thực thi SQL
  Server dùng một lần.
- Tích hợp thành phần gửi yêu cầu FE02 FE10-S04 và tích hợp kết quả tư cách thành viên
  FE04 FE10-S09 đã được tích hợp vào worktree này.
- Xác thực tập trung FE02/FE10/tích hợp đạt 170/170. Chấp nhận của con người,
  tích hợp PR và CI `main` hậu hợp nhất chính xác đạt cho phạm vi nhà cung cấp
  được truyền vào; việc gửi qua nhà cung cấp thực vẫn ngoài phạm vi.
...
- Bản cập nhật cơ sở dữ liệu cục bộ, API/tầng truy cập dữ liệu/frontend tập trung, tích hợp, quyền riêng tư,
  backend/frontend đầy đủ, triển khai, hệ thống, truy vết, quét phần thay đổi/bảo
  mật và bằng chứng E2E Chromium đầy đủ đều hoàn tất.
- H2 đã phê duyệt commit `b11b59e`; CI PR của commit này và bản cập nhật cơ sở dữ liệu Azure hai
  lượt chính xác đã đạt. Lượt triển khai thủ công `30305596861` đã chặn an toàn khi kiểm tra không đạt
  ở kiểm tra trước vì Windows CRLF và Linux LF tạo hash thô khác nhau.
- Một kiểm thử chính sách triển khai đã tái hiện phần xử lý đa nền tảng còn thiếu
  trước khi sửa. Sau khi sửa, hệ thống suy ra đúng mã băm cho cả dữ liệu LF và
  CRLF từ cùng nội dung UTF-8, chỉ chấp nhận mã băm do người vận hành lưu khi
  khớp một trong hai và
  từ chối thay đổi nội dung. Chính sách triển khai đạt 16/16.
- Phụ lục hash môi trường thử nghiệm đã được H2 phê duyệt và công bố trên commit PR #75
  `28c4f80`; CI đúng commit `30306805399`, triển khai Azure ưu tiên backend
...
- H3 vòng một thất bại do phát hiện ADR/trạng thái hiện tại và trạng thái giao
  ...
  phê duyệt rõ ràng. Nó hợp nhất thành
  `b75776b10d6cf4b6868d2ba51eb3268073483b8b`; CI hậu hợp nhất `30341279111`
  và môi trường thử nghiệm Azure tự động `30341540847` đã đạt.

## 9. Ma trận kiểm thử mẫu FE07 v0.6.0

- Kiểm tra cặp loại/mẫu theo nguồn, không xác định quyền sở hữu chỉ dựa vào loại.
- Bốn mẫu FE07, gửi lại cùng yêu cầu mà không tạo bản ghi hoặc lần gửi trùng.
- Điều kiện hiển thị trong hộp thư cá nhân và đường dẫn cố định `/borrowing/history`.
- Quét các trường nhạy cảm như lý do từ chối, email, mã thông báo, OTP, dấu vết
  ngăn xếp và chi tiết nhà cung cấp.
- Cô lập lỗi FE10 khi tích hợp FE07/FE08 và kiểm tra điều hướng trên giao diện máy tính.

Bản cập nhật cơ sở dữ liệu phải chứng minh dữ liệu mẫu Azure SQL chỉ được bổ
sung và có thể chạy lại an toàn.

## 10. Bằng chứng H2 cục bộ v0.6.0

- FE10 tập trung: 4 bộ, 175/175 kiểm thử cho cặp loại/mẫu theo quy định, quyền sở hữu nguồn,
  hộp thư điều kiện hiển thị và bản cập nhật cơ sở dữ liệu gửi lại cùng yêu cầu.
- Tích hợp hệ thống đạt 11/11; kiểm thử liên hoàn Chromium đạt 1/1 và tạo đúng bốn thông báo,
  gửi lại cùng yêu cầu không tăng số bản ghi và đường dẫn thao tác cố định mở đúng nghiệp vụ.
- Toàn bộ backend đạt 1.125/1.125; độ bao phủ nhánh thấp nhất là 80,72%; triển khai
  20/20; Azure-compatible cấu trúc cơ sở dữ liệu sinh thành công.
- SHA-256 bản cập nhật cơ sở dữ liệu:
  `680cb4acc2ac3d38740e68026f29753a28a39daceeba647d24dc5e1523f3475a`.
- Truy vết FE10 14/20 FR được tag (70%), đạt ngưỡng enforce.
- Phần thay đổi của sản phẩm và bản cập nhật cơ sở dữ liệu của đợt này sau đó đã qua H2/H3, hợp nhất; hoàn tất
  cuối được xác nhận qua PR #89, CI và Azure đúng commit.
