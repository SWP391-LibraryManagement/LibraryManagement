...
> rộng hộp thư cá nhân v0.5.0 đã hoàn tất. Sau cập nhật nhánh tài liệu lên
> `main@30f936d`, H2 phê duyệt mã băm nội dung
> `e123345be05b59a9e519d182b301ab5464160e8fc32aed8d17d3c463e28e0a15`.
> PR #75 tại commit `778e0a470d8a1083bf571a8007b3c058eee4bb22` vượt CI
> `30317424995`, môi trường thử nghiệm Azure `30317621429`, H3 hai trục không có phát hiện có
> thể hành động và được phê duyệt, rồi hợp nhất thành
> `b75776b10d6cf4b6868d2ba51eb3268073483b8b`. CI hậu hợp nhất `30341279111` và
> môi trường thử nghiệm Azure tự động `30341540847` đều đạt. Hoàn tất tiếp theo đã hợp nhất qua
> PR #89 thành `main@39092fb`; CI `30675444178` và môi trường thử nghiệm Azure
> `30675744992` đều đạt đúng commit. `TASKS.md` và
> `.sdd/reviews/fe10-notification-inbox-staging-h3-closeout-2026-07-27.md` là
> nguồn thông tin chính thức cho trạng thái bàn giao này. Các nhãn lịch sử `Not Started`,
> `PARTIAL` hoặc đang chờ rà soát không phải trạng thái hiện tại.
...
> `PROCESSING` đã được phê duyệt, để quyền sở hữu xử lý được cam kết trước thao tác gọi nhà cung cấp và một
...
> thoát giá trị khi chạy: mã đánh dấu không an toàn đã lưu bị từ chối trước khi tạo nội dung từ mẫu,
> lưu vào cơ sở dữ liệu hoặc gửi; các giá trị khi chạy vẫn được thoát/làm sạch.
...
> hoàn tất kiểm thử trước và sau khi sửa cùng các cổng nghiệm thu.
>
...
> bản cập nhật cơ sở dữ liệu có thể chạy lại an toàn; lần gửi nhạy cảm thành công chỉ giữ lại
> ID thông điệp của nhà cung cấp trong lịch sử lần thử; và một tiến trình SYSTEM tùy chọn xử lý các bản ghi
> `PENDING` không nhạy cảm đã xếp hàng khi backend còn hoạt động. API thủ công
> được bảo vệ và chính sách chỉ thử lại thủ công vẫn không đổi. Trên gói
> Trên gói F1 của môi trường tiền sản xuất, lịch chạy này có thể bị gián đoạn khi ứng dụng ngủ vì Always On bị tắt.
...
> phê duyệt thiết kế, SPEC và kế hoạch triển khai/H1. PR #70 về quản trị đã
> hợp nhất vào `main` tại commit `25c09ec`; FE10-I01..I08 và biện pháp khắc phục H3 vòng
> một đã được triển khai, kiểm thử và triển khai Azure. Sau cập nhật nhánh
> `main@30f936d`, các cổng tích hợp cuối đã hoàn tất qua PR #75 và bằng chứng
> đúng commit/hậu hợp nhất được ghi ở trạng thái bàn giao phía trên.
>
...
> đúng commit, H3, hợp nhất và CI/Azure sau hợp nhất đã hoàn thành qua PR #75.
...
- Gửi đồng bộ email OTP xác thực nhạy cảm qua bộ điều hợp nhà cung cấp đã cấu hình, dùng đối tượng mô phỏng được truyền vào trong kiểm thử và không lưu vào cơ sở dữ liệu nội dung nhạy cảm đã tạo từ mẫu.
...
- Giữ nội dung không nhạy cảm và mọi lần thử gửi ở trạng thái có thể truy vết mà không lưu vào cơ sở dữ liệu, ghi nhật ký, ghi kiểm toán hoặc trả về bí mật hay nội dung xác thực nhạy cảm đã tạo từ mẫu.
- Hỗ trợ tám cặp loại/mẫu đã quy định của Giai đoạn 1 cho xác minh, đặt lại mật khẩu, thiết lập tài khoản, đặt chỗ sẵn sàng, nhắc hạn trả, thông báo quá hạn, thông báo tiền phạt và kết quả tư cách thành viên.
...
| Tính năng nguồn | Tính năng hệ thống nội bộ | Tạo yêu cầu qua thành phần gửi yêu cầu được ràng buộc với `FE02`, `FE07`, `FE08`, `FE09`, `FE11` hoặc `SYSTEM`. FE02 sở hữu xác minh/đặt lại; FE11 sở hữu thiết lập tài khoản. |
| Thành phần gửi yêu cầu nguồn nội bộ | Ranh giới FE10 trong tiến trình | Ràng buộc một nguồn thuộc danh sách cho phép khi khởi tạo, từ chối ghi đè nguồn, áp dụng chính sách quyền sở hữu nguồn/loại và không phải vai trò đăng nhập. |
| Tiến trình xử lý thông báo | Thành phần hệ thống | Gửi các thông báo không nhạy cảm đã xếp hàng và ghi lại các lần thử. |
| Nhà cung cấp email | Bộ điều hợp đã cấu hình hoặc nhà cung cấp mô phỏng được truyền vào | Gửi email trong môi trường triển khai và các kiểm thử có tính xác định. |
...
- PRE-FE10-003: Loại thông báo và khóa mẫu tạo thành một cặp loại/mẫu đã quy định được phê duyệt.
- PRE-FE10-004: Kênh gửi được yêu cầu trong Giai đoạn 1 là `EMAIL`. Hộp thư web
  là giao diện trình bày cho cùng bản ghi không nhạy cảm đủ điều kiện, không phải
  kênh `IN_APP` hoặc một bản ghi gửi trùng lặp.
- PRE-FE10-005: Thiết lập nhà cung cấp email đã cấu hình nằm ngoài mã nguồn hoặc kiểm thử được cung cấp một nhà cung cấp mô phỏng được truyền vào.
- PRE-FE10-006: Các API HTTP thông báo được bảo vệ do người dùng `LIBRARIAN` hoặc `ADMIN` đã xác thực gọi cho loại thông báo không nhạy cảm, hoặc người gọi nội bộ dùng thành phần gửi yêu cầu nguồn được ràng buộc khi khởi tạo. Xác minh/đặt lại yêu cầu quyền sở hữu FE02; thiết lập tài khoản yêu cầu quyền sở hữu FE11.
...
1. FE02 tạo OTP xác minh gồm sáu chữ số, chỉ lưu hàm băm của OTP trong `AuthTokens` và nhận `tokenId` đã được lưu vào cơ sở dữ liệu.
2. FE02 yêu cầu gửi `ACCOUNT_VERIFICATION` qua thành phần gửi yêu cầu được ràng buộc với `FE02`, kèm email người nhận, mẫu chuẩn `ACCOUNT_VERIFICATION`, `otp`, `expiresInMinutes`, `sourceEntityType: AuthToken`, `sourceEntityId: tokenId` và khóa chống gửi trùng suy ra từ ID mã thông báo.
3. FE10 kiểm tra người nhận, kênh email, cặp loại/mẫu đã quy định, quyền sở hữu nguồn, tham chiếu nguồn dạng số nguyên, khóa chống gửi trùng và dữ liệu mẫu OTP bắt buộc.
4. FE10 xác nhận rằng hệ thống không chịu trách nhiệm tạo hoặc xác thực OTP.
5. FE10 lưu vào cơ sở dữ liệu siêu dữ liệu nguồn an toàn ở trạng thái `PROCESSING` trước thao tác gọi nhà cung cấp, nhưng tuyệt đối không lưu OTP hay tiêu đề/nội dung nhạy cảm đã tạo từ mẫu.
6. FE10 tạo nội dung từ mẫu rồi gửi đồng bộ thông điệp qua bộ điều hợp nhà cung cấp đã cấu hình, chỉ dùng dữ liệu thô trong bộ nhớ, sau đó ghi `SENT` hoặc `FAILED` cùng lần thử.
7. FE10 trả về `{ notificationId, status }`, trong đó trạng thái là `SENT` hoặc `FAILED`.

### MF-FE10-002: Gửi thông báo đặt lại mật khẩu

1. FE02 tạo OTP đặt lại mật khẩu gồm sáu chữ số, chỉ lưu hàm băm của OTP trong `AuthTokens` và nhận `tokenId` đã được lưu vào cơ sở dữ liệu.
2. FE02 yêu cầu gửi `PASSWORD_RESET` qua thành phần gửi yêu cầu được ràng buộc với `FE02`, kèm email người nhận, mẫu chuẩn `PASSWORD_RESET`, `otp`, `expiresInMinutes`, `sourceEntityType: AuthToken`, `sourceEntityId: tokenId` và khóa chống gửi trùng suy ra từ ID mã thông báo.
3. FE10 kiểm tra người nhận, kênh email, cặp loại/mẫu đã quy định, quyền sở hữu nguồn, tham chiếu nguồn dạng số nguyên, khóa chống gửi trùng và dữ liệu mẫu OTP bắt buộc.
...
1. FE08 Quản lý đặt chỗ yêu cầu gửi thông báo khi trạng thái đặt chỗ thay đổi hoặc một cuốn sách đã đặt chỗ trở nên sẵn có.
2. FE10 kiểm tra người nhận, kênh email, cặp `RESERVATION_AVAILABLE -> RESERVATION_READY` chuẩn và dữ liệu mẫu đặt chỗ.
3. FE10 xác nhận rằng FE08 sở hữu hàng đợi đặt chỗ và quyết định về tình trạng sẵn có.
4. FE10 từ chối đệ quy các khóa giống bí mật trong dữ liệu xếp hàng và tạo thông báo `PENDING` không nhạy cảm có nội dung đã tạo từ mẫu.
...
2. FE10 kiểm tra người nhận, kênh email, cặp loại/mẫu đã quy định và dữ liệu mẫu hạn trả/tiền phạt.
...
1. FE11 tạo mã thông báo `ACCOUNT_SETUP` an toàn về mặt mật mã, chỉ lưu hàm băm của mã thông báo trong `AuthTokens` và nhận ID mã thông báo đã được lưu vào cơ sở dữ liệu.
2. FE11 yêu cầu gửi `ACCOUNT_SETUP` qua thành phần gửi yêu cầu được ràng buộc với `FE11`, kèm email người nhận, mẫu chuẩn `ACCOUNT_SETUP`, `setupLink`, `expiresInHours`, `sourceEntityType: AuthToken`, `sourceEntityId: tokenId` và khóa chống gửi trùng `FE11:ACCOUNT_SETUP:<tokenId>`.
3. FE10 kiểm tra quyền sở hữu FE11, cặp loại/mẫu đã quy định, biến bắt buộc, ID nguồn dạng số nguyên và khả năng xử lý lặp an toàn.
4. FE10 chỉ lưu vào cơ sở dữ liệu siêu dữ liệu nguồn an toàn ở trạng thái `PROCESSING` trước thao tác gọi nhà cung cấp, còn liên kết thiết lập chỉ tồn tại trong bộ nhớ của yêu cầu/nhà cung cấp.
5. FE10 tạo nội dung từ mẫu rồi gửi đồng bộ qua bộ điều hợp nhà cung cấp đã cấu hình, sau đó ghi `SENT` hoặc `FAILED`, bản tóm tắt lỗi chung khi áp dụng và dữ liệu lần thử.
6. FE10 trả về `{ notificationId, status }` mà không trả về mã thông báo thiết lập, liên kết, tiêu đề/nội dung đã tạo từ mẫu hoặc chi tiết nhà cung cấp.

### MF-FE10-006: Xếp hàng thông báo kết quả tư cách thành viên

1. FE04 cam kết quyết định phê duyệt hoặc từ chối rồi yêu cầu `GENERAL_SYSTEM -> MEMBERSHIP_RESULT` qua thành phần gửi yêu cầu được ràng buộc với `FE04`.
2. FE10 kiểm tra quyền sở hữu FE04, người nhận, tham chiếu nguồn của hồ sơ đăng ký dạng số nguyên, cặp loại/mẫu đã quy định, dữ liệu mẫu không nhạy cảm bắt buộc và khóa chống gửi trùng.
3. FE10 tạo đúng một thông báo `PENDING` cho khóa chống gửi trùng mới và trả về `{ notificationId, status }`.
4. Sau đó, tiến trình xử lý cam kết `PROCESSING` trước thao tác gọi nhà cung cấp rồi ghi `SENT` hoặc `FAILED` cùng lần thử gửi an toàn; lỗi gửi không bao giờ thay đổi quyết định FE04 đã cam kết.
...
3. Tầng truy cập dữ liệu áp dụng quyền sở hữu, danh sách loại không nhạy cảm đủ điều kiện
...
4. FE10 chỉ trả về DTO hộp thư an toàn và `actionPath` theo danh sách cho phép
   do backend suy ra; không trả về email người nhận, dữ liệu an toàn, siêu dữ
   liệu nguồn, khóa chống gửi trùng, chi tiết nhà cung cấp, lần thử hoặc nội dung nhạy cảm.
...
3. Đánh dấu một mục trả về bản tóm tắt mục an toàn; đánh dấu tất cả trả về số
   hàng đã thay đổi.
4. Việc lặp lại mỗi thao tác đều có khả năng xử lý lặp an toàn và không bao giờ thay đổi
...
1. Tính năng nguồn gửi cùng một khóa chống gửi trùng nhiều lần.
...
2. FE10 trả về lỗi kiểm tra hợp lệ/mẫu 4xx an toàn trước khi tạo nội dung từ mẫu hoặc tạo bản ghi thông báo.
3. FE10 không lưu vào cơ sở dữ liệu nội dung yêu cầu không hợp lệ; trạng thái `FAILED` đã lưu chỉ dành cho yêu cầu được chấp nhận nhưng nhà cung cấp gửi thất bại.
4. Cặp loại/mẫu không khớp luôn bị từ chối trước khi gửi hoặc lưu vào cơ sở dữ liệu vào hàng đợi; cờ hay bí danh do người gọi cung cấp không bao giờ được dùng để chuyển đổi cặp đó.
5. FE10 không làm sạch một định nghĩa mẫu đã lưu không an toàn để biến nó thành mẫu được chấp nhận. Các giá trị khi chạy được truyền vào một định nghĩa vốn an toàn vẫn được thoát hoặc làm sạch.
...
3. FE10 ghi `FAILED`, một lần thử và lý do lỗi an toàn khi chuyển đổi sang trạng thái cuối được cam kết; không lưu vào cơ sở dữ liệu hay trả về bí mật hoặc chi tiết nhà cung cấp.
...
6. Nếu thao tác gọi nhà cung cấp hoàn tất nhưng không thể lưu vào cơ sở dữ liệu chuyển đổi sang trạng thái cuối, bản ghi vẫn ở `PROCESSING`; cả xử lý tự động và thử lại thủ công đều không được gửi lại, đồng thời thử lại thủ công trả về `409 DELIVERY_STATE_UNCERTAIN`.
...
2. FE10 trả về `403 SENSITIVE_NOTIFICATION_INTERNAL_ONLY` kèm thông báo `Sensitive authentication notifications must be requested internally.` trước khi tạo nội dung từ mẫu, lưu vào cơ sở dữ liệu hoặc gửi qua nhà cung cấp.
...
1. Frontend không thể lưu vào cơ sở dữ liệu cập nhật trạng thái đọc vì API hoặc mạng không
   khả dụng.
2. Thông báo vẫn chưa đọc và khung ứng dụng hiển thị cảnh báo an toàn không chặn.
...
- BR-FE10-002: FE10 phải kiểm tra người nhận, kênh `EMAIL`, tham chiếu nguồn dạng số nguyên, dữ liệu mẫu bắt buộc và cặp loại/mẫu đã quy định do máy chủ thực thi trước khi tạo hoặc gửi thông báo. Các cặp loại/mẫu đã quy định là `ACCOUNT_VERIFICATION -> ACCOUNT_VERIFICATION`, `PASSWORD_RESET -> PASSWORD_RESET`, `ACCOUNT_SETUP -> ACCOUNT_SETUP`, `RESERVATION_AVAILABLE -> RESERVATION_READY`, `DUE_DATE_REMINDER -> DUE_DATE_REMINDER`, `OVERDUE_NOTICE -> OVERDUE_NOTICE`, `FINE_NOTICE -> FINE_NOTICE` và `GENERAL_SYSTEM -> MEMBERSHIP_RESULT`.
- BR-FE10-003: FE10 không được tạo hoặc xác thực OTP dùng cho xác thực hay mã thông báo xác minh/đặt lại kiểu cũ.
- BR-FE10-004: FE10 không được lưu vào cơ sở dữ liệu, ghi nhật ký, ghi kiểm toán hoặc trả về mã thông báo thô, giá trị OTP, mật khẩu, liên kết xác minh/đặt lại/thiết lập, tiêu đề/nội dung xác thực nhạy cảm đã tạo từ mẫu, thông tin xác thực/chi tiết của nhà cung cấp hoặc dấu vết ngăn xếp nội bộ. `templateData` không nhạy cảm đã xếp hàng và `safePayload` đã lưu phải duyệt đệ quy qua đối tượng/mảng, đồng thời chuẩn hóa khóa bằng cách chuyển thành chữ thường và bỏ `_`, `-` cùng khoảng trắng. Yêu cầu xếp hàng bị từ chối khi khóa đã chuẩn hóa chứa `token`, `otp`, `password`, `verificationlink`, `resetlink` hoặc `setuplink`; các khóa tương tự bị che khỏi `safePayload`.
- BR-FE10-005: Mọi yêu cầu nguồn trong tiến trình phải chứa `sourceFeature`, `sourceEntityType`, `sourceEntityId` dạng số nguyên và một khóa chống gửi trùng; người gọi không thể ghi đè siêu dữ liệu đã ràng buộc. Yêu cầu HTTP dùng hợp đồng được bảo vệ riêng và không thể cung cấp `sourceFeature`.
- BR-FE10-006: Một khóa chống gửi trùng chỉ ánh xạ tới một bản ghi thông báo xuyên suốt mọi trạng thái. Yêu cầu trùng phải trả lại bản tóm tắt của bản ghi hiện có, không tạo hoặc gửi thêm thông báo.
...
- BR-FE10-010: Mẫu thông báo phải khai báo biến bắt buộc, thực thi cặp loại/mẫu đã quy định và không được âm thầm tạo nội dung khi thiếu dữ liệu bắt buộc. Định nghĩa Giai đoạn 1 là văn bản thuần cộng các mã `{{variable}}`. Trước khi tạo nội dung cho bất kỳ yêu cầu nào, FE10 phải từ chối tiêu đề/nội dung đã lưu có cú pháp thẻ HTML thô (bao gồm `<script>`), thuộc tính xử lý sự kiện nội tuyến hoặc URL `javascript:`, thay vì làm sạch định nghĩa đó thành một mẫu được chấp nhận. Các giá trị được chèn vào một định nghĩa vốn an toàn vẫn phải được thoát hoặc làm sạch. Cả `ACCOUNT_VERIFICATION` và `PASSWORD_RESET` đều yêu cầu `otp` cùng `expiresInMinutes`; `ACCOUNT_SETUP` yêu cầu `setupLink` và `expiresInHours`.
...
- BR-FE10-013: Thay đổi trạng thái thông báo và bản ghi kiểm toán yêu cầu nguồn phải có khả năng truy vết bằng siêu dữ liệu an toàn. Bản ghi kiểm toán nguồn dùng `userId: null` cùng siêu dữ liệu nguồn đã ràng buộc; thao tác thử lại giữ nguyên ID thông báo, khóa chống gửi trùng và lịch sử lần thử. Việc nhận xử lý phải cam kết nguyên tử `PENDING -> PROCESSING` trước thao tác gọi nhà cung cấp, còn chuyển đổi sang trạng thái cuối phải được bảo vệ từ `PROCESSING`.
- Kết quả thành công của nhà cung cấp chỉ lưu vào cơ sở dữ liệu giá trị đã chuẩn hóa
  `providerMessageId` trong `NotificationAttempts`; không sao chép toàn bộ phản hồi của nhà cung cấp,
  nội dung nhạy cảm đã tạo từ mẫu, mã thông báo, OTP, liên kết thiết lập hoặc nội dung của người nhận
...
- BR-FE10-015: Điều kiện hiển thị trong hộp thư yêu cầu `UserId` khác null đã
  được lưu vào cơ sở dữ liệu và loại chuẩn không nhạy cảm. `ACCOUNT_VERIFICATION`,
...
- BR-FE10-016: Trạng thái đọc của hộp thư là `ReadAt IS NULL` khi chưa đọc và
  ...
  phải có khả năng xử lý lặp an toàn.
- BR-FE10-017: FE10 chỉ được suy ra `actionPath` từ tổ hợp loại/mẫu/nguồn chuẩn
  đã lưu vào cơ sở dữ liệu và danh sách đường dẫn tương đối cho phép cố định. Không bên gọi
  nguồn, mẫu, dữ liệu, trường nội dung cơ sở dữ liệu hoặc dữ liệu đầu vào
...
- BR-FE10-019: Bản cập nhật cơ sở dữ liệu bổ sung phải đánh dấu các hàng đủ điều kiện cho hộp thư
  đã tồn tại trước khi triển khai là đã đọc bằng `ReadAt = CreatedAt`; chỉ các
  thông báo đủ điều kiện được tạo sau bản cập nhật cơ sở dữ liệu mới bắt đầu ở trạng thái chưa đọc.
...
- FR-FE10-001: Khi thành phần gửi yêu cầu được ràng buộc với `FE02` gửi dữ liệu OTP xác minh tài khoản chuẩn gồm `otp`, `expiresInMinutes` và tham chiếu nguồn `AuthToken`, FE10 phải chỉ lưu vào cơ sở dữ liệu siêu dữ liệu nguồn an toàn dưới trạng thái `PROCESSING` trước thao tác gọi nhà cung cấp, tạo nội dung từ mẫu rồi gửi đồng bộ qua bộ điều hợp nhà cung cấp đã cấu hình, ghi bản tóm tắt `SENT` hoặc `FAILED` đã che dữ liệu cùng lần thử khi chuyển đổi trạng thái cuối được cam kết, rồi trả về `{ notificationId, status }` mà không tạo, xác thực, lưu vào cơ sở dữ liệu, ghi nhật ký, ghi kiểm toán hoặc trả về OTP.
- FR-FE10-002: Khi thành phần gửi yêu cầu được ràng buộc với `FE02` gửi dữ liệu OTP đặt lại mật khẩu chuẩn gồm `otp`, `expiresInMinutes` và tham chiếu nguồn `AuthToken`, FE10 phải chỉ lưu vào cơ sở dữ liệu siêu dữ liệu nguồn an toàn dưới trạng thái `PROCESSING` trước thao tác gọi nhà cung cấp, tạo nội dung từ mẫu rồi gửi đồng bộ qua bộ điều hợp nhà cung cấp đã cấu hình, ghi bản tóm tắt `SENT` hoặc `FAILED` đã che dữ liệu cùng lần thử khi chuyển đổi trạng thái cuối được cam kết, rồi trả về `{ notificationId, status }` mà không làm lộ nội dung nhạy cảm thô hay đã tạo từ mẫu.
...
- FR-FE10-005: Khi kiểm tra trường bắt buộc, tham chiếu nguồn dạng số nguyên, ánh xạ chuẩn, người nhận, quyền sở hữu nguồn/loại, ghi đè nguồn qua HTTP, độ an toàn của dữ liệu xếp hàng hoặc độ an toàn của định nghĩa mẫu đã lưu không đạt, FE10 phải từ chối yêu cầu an toàn trước khi tạo nội dung từ mẫu, lưu vào cơ sở dữ liệu hoặc gửi.
...
- FR-FE10-008: Khi sự kiện nguồn trùng được gửi với cùng khóa chống gửi trùng, FE10 phải trả về `200 { notificationId, status }` cho bản ghi hiện có ở bất kỳ trạng thái nào và không được tạo hoặc gửi bản trùng.
- FR-FE10-009: FE10 phải nhận biết đủ tám cặp loại/mẫu đã quy định, gồm `ACCOUNT_SETUP -> ACCOUNT_SETUP` và `GENERAL_SYSTEM -> MEMBERSHIP_RESULT`. Mẫu bị thiếu/không hoạt động, định nghĩa mẫu đã lưu không an toàn, biến nhạy cảm bắt buộc bị thiếu, cặp không khớp, nguồn nhạy cảm không được cấp quyền, ghi đè nguồn qua HTTP hoặc khóa giống bí mật trong dữ liệu xếp hàng được phát hiện đệ quy phải trả về lỗi 4xx an toàn trước khi tạo nội dung từ mẫu hoặc lưu vào cơ sở dữ liệu mà không làm lộ giá trị đã gửi.
- FR-FE10-010: Khi thành phần gửi yêu cầu được ràng buộc với `FE11` gửi dữ liệu thiết lập tài khoản chuẩn gồm `setupLink`, `expiresInHours` và tham chiếu nguồn `AuthToken`, FE10 phải lưu vào cơ sở dữ liệu siêu dữ liệu nguồn an toàn dưới trạng thái `PROCESSING` trước thao tác gọi nhà cung cấp, tạo nội dung từ mẫu rồi gửi đồng bộ, ghi trạng thái cuối và siêu dữ liệu lần thử an toàn khi chuyển đổi được cam kết, rồi trả về `{ notificationId, status }` mà không làm lộ nội dung thiết lập thô hay đã tạo từ mẫu.
...
- FR-FE10-013: Khi tác nhân đã xác thực đánh dấu đã đọc một thông báo đủ điều
  ...
  cùng một `404` không thể phân biệt và việc gọi lại thao tác phải cho cùng kết quả an toàn.
- FR-FE10-014: Khi tác nhân đã xác thực đánh dấu đã đọc tất cả thông báo cá nhân,
  ...
  của tác nhân đó và trả về `{ updated }`; khi gọi lại thao tác, hệ thống phải trả về `updated: 0`.
- FR-FE10-015: Khi FE10 chiếu một thông báo đủ điều kiện, hệ thống phải suy ra
  ...
  không chấp nhận URL nội dung do bên gọi kiểm soát hoặc đã lưu vào cơ sở dữ liệu.
- FR-FE10-016: khung ứng dụng ứng dụng đã xác thực phải hiển thị chuông với huy hiệu số
  ...
  cửa sổ nhận tiêu điểm, thao tác đọc và mỗi 60 giây trong khi khung ứng dụng được gắn.

---

## 8. Tiêu chí chấp nhận

- AC-FE10-001: Với việc thành phần gửi yêu cầu được ràng buộc với `FE02` gửi dữ liệu OTP xác minh tài khoản chuẩn, khi FE10 gửi đồng bộ thì hệ thống trả về `{ notificationId, status }` với `SENT` hoặc `FAILED`, lưu vào cơ sở dữ liệu siêu dữ liệu nguồn `AuthToken` an toàn và không lưu vào cơ sở dữ liệu OTP hay nội dung nhạy cảm đã tạo từ mẫu.
- AC-FE10-002: Với việc thành phần gửi yêu cầu được ràng buộc với `FE02` gửi dữ liệu OTP đặt lại mật khẩu chuẩn, khi FE10 gửi đồng bộ thì hệ thống trả về `{ notificationId, status }` với `SENT` hoặc `FAILED`, lưu vào cơ sở dữ liệu siêu dữ liệu nguồn `AuthToken` an toàn và không lưu vào cơ sở dữ liệu OTP hay nội dung nhạy cảm đã tạo từ mẫu.
...
- AC-FE10-006: Với từng cặp trong tám cặp loại/mẫu đã quy định, gồm `ACCOUNT_SETUP -> ACCOUNT_SETUP` và `GENERAL_SYSTEM -> MEMBERSHIP_RESULT`, khi FE10 kiểm tra một yêu cầu đầy đủ từ ranh giới được cấp quyền cùng định nghĩa mẫu đã lưu an toàn, thì kiểm tra ánh xạ đạt và các giá trị khi chạy được thoát/làm sạch. Với người nhận/biến bị thiếu, ID nguồn dạng chuỗi, cặp không khớp, mẫu không rõ/không hoạt động, định nghĩa mẫu đã lưu không an toàn, ghi đè nguồn qua HTTP, nguồn nhạy cảm không được cấp quyền hoặc khóa bí mật lồng nhau trong dữ liệu xếp hàng, việc kiểm tra trả về lỗi 4xx an toàn trước khi tạo nội dung từ mẫu, lưu vào cơ sở dữ liệu, tạo lần thử hoặc gửi.
- AC-FE10-007: Với việc FE02 cung cấp OTP qua thành phần gửi yêu cầu đã ràng buộc, khi FE10 gửi thì OTP cùng tiêu đề/nội dung nhạy cảm đã tạo từ mẫu không xuất hiện trong dữ liệu lưu vào cơ sở dữ liệu, nhật ký, bản ghi kiểm toán hay phản hồi HTTP.
- AC-FE10-008: Với một khóa chống gửi trùng đã tồn tại ở bất kỳ trạng thái nào, khi FE10 nhận yêu cầu trùng thì hệ thống trả về `200 { notificationId, status }` cho bản ghi đó và không gửi trùng.
- AC-FE10-009: Khi nhà cung cấp gửi thất bại và FE10 ghi nhận lỗi, luồng nguồn vẫn hoàn tất; bản ghi không nhạy cảm đã xếp hàng nhưng thất bại có thể thử lại trên cùng lịch sử, còn thông báo nhạy cảm trả về lỗi an toàn `409 REISSUE_REQUIRED`. Nếu nhà cung cấp đã xử lý nhưng hệ thống không lưu được trạng thái cuối vào cơ sở dữ liệu, bản ghi vẫn ở `PROCESSING`; yêu cầu trùng không gửi lại và thao tác thử lại trả về `409 DELIVERY_STATE_UNCERTAIN`.
- AC-FE10-010: Với việc thành phần gửi yêu cầu được ràng buộc với `FE11` gửi dữ liệu thiết lập tài khoản chuẩn, khi FE10 gửi đồng bộ thì hệ thống trả về bản tóm tắt `SENT`/`FAILED` an toàn, lưu vào cơ sở dữ liệu siêu dữ liệu `AuthToken` an toàn và không lưu vào cơ sở dữ liệu hay trả về mã thông báo/liên kết/nội dung thiết lập đã tạo từ mẫu.
...
- AC-FE10-014: Cho trước nhiều hàng đủ điều kiện chưa đọc của một người dùng và
  ...
  máy chủ; khi gọi lại thao tác, hệ thống trả về `updated: 0`.
- AC-FE10-015: Cho trước mọi loại chuẩn đủ điều kiện, khi FE10 trả về một mục hộp
  ...
  ghi đè giá trị này.
- AC-FE10-016: Cho trước khung ứng dụng đã xác thực và trang hộp thư, khi thông báo được
...
| EC-FE10-008 | Khóa chống gửi trùng bị trùng ở bất kỳ trạng thái nào | Trả về `200 { notificationId, status }` cho bản ghi hiện có mà không gửi trùng. |
...
| EC-FE10-010 | Tiêu đề/nội dung mẫu đã lưu chứa cú pháp thẻ HTML thô (gồm `<script>`), thuộc tính xử lý sự kiện nội tuyến hoặc URL `javascript:` | Từ chối định nghĩa mẫu bằng lỗi kiểm tra hợp lệ an toàn trước khi tạo nội dung từ mẫu, lưu vào cơ sở dữ liệu thông báo/lần thử hoặc gửi qua nhà cung cấp; không được âm thầm làm sạch rồi chấp nhận định nghĩa. Các giá trị khi chạy trong mẫu văn bản thuần cộng biến vốn an toàn vẫn được thoát/làm sạch. |
| EC-FE10-011 | Giao dịch nguồn hoàn tất nhưng gửi thất bại hoặc thành phần gửi yêu cầu ném lỗi | Giữ giao dịch nguồn ở trạng thái hoàn tất; ghi nhận/xử lý an toàn lỗi FE10. Chỉ bản ghi không nhạy cảm đã xếp hàng nhưng thất bại mới được thử lại về `PENDING`. |
...
| EC-FE10-013 | Nhân viên qua HTTP hoặc thành phần gửi yêu cầu không có quyền sở hữu gửi thông báo xác thực nhạy cảm | Trả về lỗi an toàn `403 SENSITIVE_NOTIFICATION_INTERNAL_ONLY`; FE02 sở hữu riêng việc xác minh/đặt lại và FE11 sở hữu riêng việc thiết lập tài khoản. |
| EC-FE10-014 | Lần gửi lại của FE02 phát hành mã thông báo OTP mới | Dùng `AuthTokens.TokenId` mới trong khóa chống gửi trùng mới; không dùng lại thông báo OTP trước đó. |
...
| EC-FE10-016 | Gửi lại FE11 tạo mã thông báo thiết lập mới | Dùng khóa `AuthTokens.TokenId` và `FE11:ACCOUNT_SETUP:<tokenId>` mới; không bao giờ dùng lại liên kết thiết lập trước đó. |
| EC-FE10-017 | Có kết quả từ nhà cung cấp nhưng không lưu được trạng thái cuối/lần thử vào cơ sở dữ liệu | Giữ bản ghi đã cam kết ở `PROCESSING`; không tự động nhận lại hoặc gửi lại; yêu cầu trùng trả về cùng bản tóm tắt và thao tác thử lại thủ công trả về `409 DELIVERY_STATE_UNCERTAIN`. |
...
| EC-FE10-022 | Thao tác đánh dấu một hoặc đánh dấu tất cả được gọi lại | Giữ dấu thời gian đọc đầu tiên; trả về cùng mục đã đọc an toàn hoặc `updated: 0`; không tạo lần thử gửi. |
| EC-FE10-023 | Siêu dữ liệu đã lưu vào cơ sở dữ liệu không có ánh xạ điều hướng được phê duyệt | Trả về `actionPath: null`; không bao giờ tạo đường dẫn từ tiêu đề, nội dung, dữ liệu hoặc dữ liệu đầu vào tùy ý. |
...
| NotificationTemplates | Lưu các mẫu email đã phê duyệt và biến bắt buộc. Tiêu đề/nội dung đã lưu phải đạt kiểm tra an toàn trước mỗi lần được dùng để tạo thông báo. |
| Notifications | Lưu tham chiếu nguồn, trạng thái gửi, dữ liệu an toàn, nội dung không nhạy cảm đã tạo từ mẫu, bản tóm tắt nhạy cảm đã che dữ liệu và trạng thái đọc cá nhân có thể null. |
...
| recipientEmail | string | Có điều kiện | Bắt buộc để gửi email và được lưu vào cơ sở dữ liệu dưới dạng `Notifications.RecipientEmail NVARCHAR(255) NOT NULL` nhằm khớp độ rộng email FE02/FE11. |
| loại | enum | Có | Các giá trị: `ACCOUNT_VERIFICATION`, `PASSWORD_RESET`, `ACCOUNT_SETUP`, `RESERVATION_AVAILABLE`, `DUE_DATE_REMINDER`, `OVERDUE_NOTICE`, `FINE_NOTICE`, `GENERAL_SYSTEM`. |
...
| title | string | Không | Chỉ là tiêu đề đã tạo từ mẫu của thông báo không nhạy cảm đã xếp hàng; tiêu đề xác thực nhạy cảm không được lưu vào cơ sở dữ liệu. |
| body | string | Không | Chỉ là nội dung đã tạo từ mẫu của thông báo không nhạy cảm đã xếp hàng; nội dung xác thực nhạy cảm không được lưu vào cơ sở dữ liệu. Không được chứa mã lệnh không an toàn. |
...
| status | enum | Có | Vòng đời Giai đoạn 1 dùng `PENDING`, `PROCESSING`, `SENT` và `FAILED`. Việc tạo thông báo nhạy cảm lưu vào cơ sở dữ liệu `PROCESSING` trước thao tác gọi nhà cung cấp; thông báo không nhạy cảm bắt đầu ở `PENDING` và quyền nhận xử lý của tiến trình cam kết `PROCESSING`; chuyển đổi trạng thái cuối được bảo vệ từ `PROCESSING`; thử lại chỉ cho phép thông báo không nhạy cảm chuyển `FAILED -> PENDING`. `DELIVERED`, `SKIPPED` và `CANCELLED` chỉ được giữ làm giá trị tương thích cơ sở dữ liệu và không có chuyển đổi nào trong Giai đoạn 1. |
...
| sourceEntityId | integer | Bắt buộc với yêu cầu trong tiến trình | Tham chiếu dương của Giai đoạn 1 tới bản ghi nguồn; giá trị bị thiếu và chuỗi đều bị từ chối. Yêu cầu xác thực nhạy cảm dùng `AuthTokens.TokenId` đã lưu vào cơ sở dữ liệu. |
...
| sentAt | datetime | Không | Dấu thời gian máy chủ được đặt khi nhà cung cấp email Giai đoạn 1 chấp nhận lần gửi và hệ thống lưu thành công trạng thái cuối; là null khi ở `PENDING`/`PROCESSING` và sau lần thử thất bại. |
| readAt | datetime | Không | Trạng thái đọc của hộp thư cá nhân, có thể null với bản ghi không nhạy cảm đủ điều kiện và có `userId` khác null. Thông báo đủ điều kiện mới bắt đầu bằng null; thao tác đọc thành công đầu tiên đặt dấu thời gian máy chủ. Các bản ghi lịch sử đủ điều kiện được cập nhật bằng `createdAt`; bản ghi nhạy cảm và bản ghi chỉ gửi email không gắn người dùng không xuất hiện trong hộp thư. |
...
- Thông báo xác thực/thiết lập nhạy cảm: `[*] -> PROCESSING -> SENT` hoặc `[*] -> PROCESSING -> FAILED`; thử lại luôn yêu cầu sự kiện nguồn mới và khóa chống gửi trùng mới.
- Một hàng còn ở `PROCESSING` sau thao tác gọi nhà cung cấp có trạng thái gửi không chắc chắn. Hàng đó bị loại khỏi các lượt nhận xử lý của tiến trình và mọi đường thử lại để FE10 không thể gửi trùng.
...
| ------ | -------- | ----- | ------- | -------- | ----- |
| POST | `/api/notifications/requests` | `LIBRARIAN`, `ADMIN` | `{ type, channel, userId?, recipientEmail?, templateKey, templateData, sourceEntityType?, sourceEntityId?, idempotencyKey? }` | Yêu cầu mới: `201 { notificationId, status }`; yêu cầu trùng theo khóa chống gửi trùng: `200 { notificationId, status }`; loại nhạy cảm: lỗi an toàn `403` | Ranh giới HTTP cho thông báo không nhạy cảm được bảo vệ bằng vai trò. Không chấp nhận `sourceFeature`. Mọi loại xác thực nhạy cảm đều trả về `SENSITIVE_NOTIFICATION_INTERNAL_ONLY`. Tuyệt đối không trả về toàn bộ nội dung hoặc `safePayload`. |
...
| PATCH | `/api/notifications/{id}/read` | `MEMBER`, `LIBRARIAN`, `ADMIN` đã xác thực | Không có | `200 SafeInboxItem`; không tồn tại/nhạy cảm/không thuộc sở hữu: `404` an toàn | Có khả năng xử lý lặp an toàn; không thể thay đổi trạng thái gửi hoặc các lần thử. |
| PATCH | `/api/notifications/mine/read-all` | `MEMBER`, `LIBRARIAN`, `ADMIN` đã xác thực | Không có | `200 { updated }` | Dùng một dấu thời gian máy chủ cho các bản ghi đủ điều kiện chưa đọc của chính mình; gọi lại thao tác trả về 0. |

Lỗi kiểm tra hợp lệ và lỗi mẫu dùng nội dung phản hồi 4xx an toàn. Trạng thái thử lại không hợp lệ dùng nội dung phản hồi `409` an toàn. Không phản hồi nào chứa nội dung đã tạo từ mẫu, bí mật đầu vào thô, chi tiết nhà cung cấp hoặc dấu vết ngăn xếp nội bộ.
...
- NFR-FE10-SEC-004: FE10 không được lưu vào cơ sở dữ liệu, ghi nhật ký, ghi kiểm toán hoặc trả về mã thông báo thô, OTP, mật khẩu, liên kết đặt lại/xác minh/thiết lập, nội dung xác thực nhạy cảm đã tạo từ mẫu, thông tin xác thực/chi tiết của nhà cung cấp hoặc dấu vết ngăn xếp nội bộ.
- NFR-FE10-SEC-005: Kiểm tra độ an toàn của mẫu và làm sạch giá trị được chèn vào mẫu là hai bước bảo mật riêng. FE10 phải từ chối cú pháp thẻ HTML thô, thuộc tính xử lý sự kiện nội tuyến và URL `javascript:` trong tiêu đề/nội dung mẫu đã lưu trước khi tạo nội dung từ mẫu, lưu vào cơ sở dữ liệu hoặc gửi. Các giá trị được chèn vào một mẫu văn bản thuần vốn an toàn vẫn phải được thoát hoặc làm sạch.
...
- NFR-FE10-SEC-007: Việc phân quyền hộp thư cá nhân phải được thực thi trong
  truy vấn tầng truy cập dữ liệu/tầng dịch vụ của backend bằng `UserId` đã xác thực; cơ chế bảo
...
- NFR-FE10-REL-003: Sự kiện nguồn trùng phải trả về cùng một bản ghi xuyên suốt mọi trạng thái. Thao tác thử lại thủ công với thông báo không nhạy cảm phải giữ nguyên ID thông báo, khóa chống gửi trùng và lịch sử lần thử. Chỉ được gọi nhà cung cấp sau khi quyền xử lý `PROCESSING` đã được lưu vào cơ sở dữ liệu; bản ghi `PROCESSING` có kết quả không chắc chắn tuyệt đối không được tự động nhận lại.
- NFR-FE10-REL-004: Thao tác thay đổi trạng thái đọc phải có khả năng xử lý lặp an toàn, giữ
...
- NFR-FE10-LOG-002: Nhật ký và bản ghi kiểm toán phải lưu bản tóm tắt an toàn, không lưu bí mật, nội dung xác thực nhạy cảm đã tạo từ mẫu hay phản hồi thô của nhà cung cấp. Bản ghi kiểm toán nguồn nội bộ dùng `userId: null` cùng siêu dữ liệu nguồn đã ràng buộc.
...
- NFR-FE10-UX-003: khung ứng dụng đã xác thực phải cung cấp chuông có thể truy cập bằng
...
- WebSocket, sự kiện do máy chủ đẩy (Server-Sent Events), thông báo đẩy qua tiến trình xử lý nền hoặc kênh gửi
  `IN_APP` riêng.
- URL điều hướng thông báo do bên gọi cung cấp hoặc được suy ra từ nội dung đã lưu vào cơ sở dữ liệu.
...
| ---------- | ---- | ----- |
| Xác thực FE02 | Nội bộ | Sở hữu việc tạo và xác thực OTP/mã thông báo, sau đó yêu cầu gửi OTP xác minh tài khoản và đặt lại mật khẩu qua thành phần gửi yêu cầu được ràng buộc với `FE02`. Việc chấp nhận mã thông báo kiểu cũ và `CHANGE_PASSWORD_OTP` chỉ để tương thích vẫn thuộc FE02. |
...
| Quản lý tư cách thành viên FE04 | Nội bộ | Sở hữu sự kiện nguồn `MEMBERSHIP_RESULT` và dùng thành phần gửi yêu cầu được ràng buộc với FE04 sau khi cam kết quyết định rà soát. |
...
| Quản lý người dùng và vai trò FE11 | Nội bộ | Sở hữu việc phát hành/gửi lại mã thông báo thiết lập do Quản trị viên tạo và yêu cầu `ACCOUNT_SETUP` chuẩn qua thành phần gửi yêu cầu được ràng buộc với `FE11`. |
| Cơ sở dữ liệu SQL Server | Kỹ thuật | Lưu bản ghi thông báo, mẫu, lần thử và tùy chọn. |
| Bộ điều hợp nhà cung cấp email đã cấu hình hoặc nhà cung cấp mô phỏng được truyền vào | Kỹ thuật | Gửi thông báo email; môi trường triển khai dùng thiết lập nhà cung cấp đã cấu hình, còn kiểm thử chèn một đối tượng mô phỏng có tính xác định. |
| Bộ lập lịch/tiến trình xử lý | Kỹ thuật | Dùng ranh giới nguồn nội bộ `SYSTEM` và chỉ xử lý thông báo `PENDING` không nhạy cảm. |
| khung ứng dụng ứng dụng đã xác thực | Kỹ thuật | Chứa chuông cá nhân, phần xem trước, thăm dò số lượng chưa đọc và tuyến `/notifications` cho mọi vai trò đăng nhập. |
...
| Q-FE10-007 | Hệ thống/Bộ lập lịch có thể kích hoạt qua thành phần gửi yêu cầu được ràng buộc với `SYSTEM`; nguồn nội bộ thuộc danh sách cho phép và không phải vai trò đăng nhập. | Phê duyệt G3 2026-07-13 | APPROVED |
| Q-FE10-008 | `ACCOUNT_SETUP` là thao tác gửi nhạy cảm thuộc FE11; chỉ thành phần gửi yêu cầu được ràng buộc với FE11 mới được gửi và FE10 không lưu vào cơ sở dữ liệu mã thông báo/liên kết/nội dung đã tạo từ mẫu của quá trình thiết lập. | ADR-005; Nhat phê duyệt 2026-07-15 | APPROVED |
| Q-FE10-009 | `MEMBERSHIP_RESULT` thuộc FE04; FE04 gửi qua thành phần gửi yêu cầu được ràng buộc với FE04 sau khi quyết định tư cách thành viên được cam kết. | Kiểm toán liên tính năng FE04 2026-07-17 | APPROVED |
| Q-FE10-010 | Các trạng thái thông báo Giai đoạn 1 là `PENDING`, `PROCESSING`, `SENT` và `FAILED`; quyền nhận xử lý/chấp nhận thông báo nhạy cảm cam kết `PROCESSING` trước thao tác gọi nhà cung cấp, còn các trạng thái tương thích không có chuyển đổi trong Giai đoạn 1. | Chuẩn hóa vòng đời thông báo 2026-07-17; biện pháp khắc phục an toàn gửi được phê duyệt 2026-07-23 | APPROVED |
...
| Q-FE10-012 | FE10 làm sạch hay từ chối một định nghĩa mẫu đã lưu không an toàn? | Nhat, 2026-07-27 | APPROVED: từ chối định nghĩa đã lưu trước khi tạo nội dung từ mẫu/lưu vào cơ sở dữ liệu/gửi; tiếp tục thoát hoặc làm sạch giá trị khi chạy. |
| Q-FE10-013 | Môi trường tiền sản xuất dùng tiến trình nền SYSTEM chạy cùng ứng dụng, có cơ chế bật tùy chọn, chu kỳ mặc định 60 giây và kích thước lô 20. Tiến trình chạy một lần sau khi khởi động, ngăn các lượt chạy cục bộ chồng lấp, chỉ xử lý bản ghi `PENDING` không nhạy cảm và dừng cùng máy chủ HTTP. Endpoint nhân viên hiện có vẫn được bảo vệ và trạng thái `FAILED` chỉ được thử lại thủ công. Khi gói F1 ngủ, tiến trình cũng tạm dừng. | Phê duyệt của người dùng và thiết kế bằng văn bản 2026-07-27 | APPROVED |
...
| Q-FE10-018 | Các bản ghi lịch sử đủ điều kiện được cập nhật thành đã đọc, trong khi chỉ bản ghi đủ điều kiện tạo sau bản cập nhật cơ sở dữ liệu mới bắt đầu ở trạng thái chưa đọc. | Thiết kế hộp thư và SPEC bằng văn bản được người dùng phê duyệt ngày 2026-07-27 | APPROVED |
...
| -------- | --------------- | ------ |
| Q-FE10-001 | Kênh bắt buộc của Giai đoạn 1 là email qua bộ điều hợp nhà cung cấp đã cấu hình; kiểm thử dùng nhà cung cấp mô phỏng được truyền vào. | APPROVED |
| Q-FE10-002 | v0.5.0 thay thế quyết định hoãn hộp thư bằng dữ liệu hiển thị cá nhân từ các bản ghi không nhạy cảm đủ điều kiện hiện có; kênh gửi vẫn chỉ dùng email. | APPROVED 2026-07-27 |
...
| Q-FE10-007 | Hệ thống/Bộ lập lịch dùng thành phần gửi yêu cầu nội bộ đã ràng buộc và không phải vai trò đăng nhập. | APPROVED |
| Q-FE10-008 | Thiết lập tài khoản là thao tác gửi nhạy cảm đồng bộ thuộc quyền sở hữu của thành phần gửi yêu cầu được ràng buộc với FE11. | APPROVED |
| Q-FE10-013 | Tiến trình SYSTEM có thể bật để chạy lúc khởi động và mặc định cứ 60 giây một lần với kích thước lô 20; ngăn chạy cục bộ chồng lấp, dừng theo vòng đời ứng dụng, giữ nguyên phân quyền HTTP của nhân viên, chỉ thử lại lỗi thủ công và có thể bị gián đoạn khi gói F1 ngủ. | APPROVED 2026-07-27 |
| G1 | Thông báo xác thực nhạy cảm được gửi đồng bộ mà không lưu vào cơ sở dữ liệu nội dung đã tạo từ mẫu; thông báo không nhạy cảm vẫn được xếp hàng với cơ chế đệ quy bảo vệ khóa bí mật đã chuẩn hóa và che dữ liệu `safePayload` tương ứng. | APPROVED 2026-07-13 |
| G2 | Các thao tác tạo/yêu cầu trùng/xử lý/thử lại chỉ trả về DTO tối thiểu đã phê duyệt. | APPROVED 2026-07-13 |
...
| G6 | Mỗi khóa chống gửi trùng có một bản ghi xuyên suốt mọi trạng thái; thao tác thử lại dùng lại cùng lịch sử. | APPROVED 2026-07-13 |
...
| G8 | FE02 tạo và xác thực OTP gồm sáu chữ số; FE10 tạo nội dung từ mẫu rồi gửi `otp` cùng `expiresInMinutes` qua thành phần gửi yêu cầu được ràng buộc với `FE02` mà không lưu vào cơ sở dữ liệu hay làm lộ OTP. | APPROVED 2026-07-15 |
| G9 | Nhân viên qua HTTP và thành phần gửi yêu cầu nguồn không phải FE02 không thể gửi `ACCOUNT_VERIFICATION` hoặc `PASSWORD_RESET`; HTTP không thể cung cấp `sourceFeature`; vi phạm trả về lỗi an toàn `403 SENSITIVE_NOTIFICATION_INTERNAL_ONLY`. | APPROVED 2026-07-15 |
| G10 | Khả năng xử lý lặp an toàn và khả năng truy vết nguồn cho thông báo nhạy cảm dùng `AuthTokens.TokenId`; FE02 loại bỏ việc ghi thông báo xác minh/đặt lại trùng và gửi trực tiếp, đồng thời lỗi vẫn không chặn luồng và gửi lại sẽ tạo sự kiện nguồn mới. | APPROVED 2026-07-15 |
...
| G12 | FE04 sở hữu sự kiện nguồn `MEMBERSHIP_RESULT`; FE10 chỉ chấp nhận từ thành phần gửi yêu cầu được ràng buộc với FE04 và giữ lỗi gửi ở trạng thái không chặn luồng. | APPROVED 2026-07-17 |
| Q-FE10-010 | Các trạng thái Giai đoạn 1 là `PENDING`, `PROCESSING`, `SENT` và `FAILED`; `PROCESSING` được lưu vào cơ sở dữ liệu trước thao tác gọi nhà cung cấp và không bao giờ được tự động nhận lại. | APPROVED; sửa đổi 2026-07-23 |
| Q-FE10-011 | FE04 sử dụng `GENERAL_SYSTEM -> MEMBERSHIP_RESULT`; FE08 sử dụng `RESERVATION_AVAILABLE -> RESERVATION_READY`. | APPROVED |
| Q-FE10-012 | Định nghĩa mẫu đã lưu không an toàn bị từ chối trước khi tạo nội dung từ mẫu/lưu vào cơ sở dữ liệu/gửi; giá trị khi chạy vẫn được thoát/làm sạch. | APPROVED 2026-07-27 |
...
| Q-FE10-018 | Bản cập nhật cơ sở dữ liệu đánh dấu các hàng lịch sử đủ điều kiện là đã đọc. | APPROVED 2026-07-27 |
...
| ----- | -------------------- | ---------- | ---------- | --------------- | -------------- | ------ |
| AC-FE10-001 | OTP xác minh tài khoản từ thành phần gửi yêu cầu ràng buộc với FE02 được gửi đồng bộ bằng siêu dữ liệu nguồn an toàn và không lưu vào cơ sở dữ liệu nội dung nhạy cảm | FR-FE10-001 | BR-FE10-001 đến BR-FE10-004, BR-FE10-005, BR-FE10-007 đến BR-FE10-011 | FT46 | FE10-S01 đến FE10-S04 | Hoàn thành |
| AC-FE10-002 | OTP đặt lại mật khẩu từ thành phần gửi yêu cầu ràng buộc với FE02 được gửi đồng bộ bằng siêu dữ liệu nguồn an toàn và không lưu vào cơ sở dữ liệu nội dung nhạy cảm | FR-FE10-002 | BR-FE10-003 đến BR-FE10-005, BR-FE10-007 đến BR-FE10-011 | FT47 | FE10-S01 đến FE10-S04 | Hoàn thành |
| AC-FE10-003 | Thông báo kết quả tư cách thành viên FE04 và đặt chỗ sẵn sàng FE08 được xếp hàng mà FE10 không thay đổi kết quả nguồn | FR-FE10-003 | BR-FE10-001, BR-FE10-002, BR-FE10-007, BR-FE10-011, BR-FE10-012 | FT48 cùng trường hợp thành phần gửi yêu cầu FE04 đã lên kế hoạch | FE10-H02, FE10-H05, FE10-H07, G12 | Hoàn thành |
| AC-FE10-004 | Thông báo hạn trả FE07 được xếp hàng mà không thay đổi trạng thái mượn | FR-FE10-004 | BR-FE10-001, BR-FE10-002, BR-FE10-007, BR-FE10-012 | FT49 | FE10-H02, FE10-H05, FE10-H06 | Hoàn thành |
| AC-FE10-005 | Hợp đồng quá hạn/tiền phạt FE09 được phê duyệt mà FE10 không tính tiền phạt; tích hợp người gọi được trì hoãn | FR-FE10-004 | BR-FE10-001, BR-FE10-002, BR-FE10-007, BR-FE10-012 | FT49 | FE10-H01, FE10-H02, FE10-H05 | Hoàn thành |
| AC-FE10-006 | Cả tám cặp loại/mẫu đã quy định và định nghĩa an toàn đều đạt kiểm tra; người nhận, biến, ID nguồn hoặc ánh xạ không hợp lệ, mẫu không an toàn/bị thiếu/không hoạt động, quyền sở hữu nguồn, ghi đè nguồn HTTP hoặc bí mật trong dữ liệu xếp hàng được phát hiện đệ quy đều trả về lỗi 4xx an toàn trước khi tạo nội dung từ mẫu/lưu vào cơ sở dữ liệu/gửi | FR-FE10-005, FR-FE10-009 | BR-FE10-002, BR-FE10-004, BR-FE10-007, BR-FE10-010, BR-FE10-011 | FT46 đến FT49 cùng ma trận định nghĩa lưu trữ không an toàn trong `notificationRoutes.test.js` | FE10-H02, FE10-H04, FE10-S02, FE10-S06, FE10-S11 | Hoàn thành |
| AC-FE10-007 | OTP xác thực và nội dung nhạy cảm đã tạo từ mẫu không bao giờ vượt qua ranh giới lưu vào cơ sở dữ liệu/nhật ký/kiểm toán/HTTP | FR-FE10-001, FR-FE10-002 | BR-FE10-003, BR-FE10-004, BR-FE10-008, BR-FE10-013 | FT46, FT47 | FE10-H03, FE10-H04, FE10-S03 | Hoàn thành |
| AC-FE10-008 | Khóa chống gửi trùng trả về cùng một bản ghi xuyên suốt mọi trạng thái bằng DTO `200` tối thiểu | FR-FE10-008 | BR-FE10-006, BR-FE10-013 | FT46 đến FT49 | FE10-H08 | Hoàn thành |
| AC-FE10-009 | Lỗi được xử lý an toàn/không chặn; FE02 phát hành lại sự kiện OTP/mã thông báo mới, thao tác thử lại `FAILED` không nhạy cảm dùng lại lịch sử, còn `PROCESSING` không chắc chắn tuyệt đối không được gửi lại | FR-FE10-007 | BR-FE10-004, BR-FE10-008, BR-FE10-012, BR-FE10-013 | Các trường hợp nhà cung cấp/chuyển đổi/thử lại trong `backend/tests/notificationRoutes.test.js` | FE10-H03, FE10-H08, FE10-S04, FE10-S10 | Hoàn thành |
| AC-FE10-010 | Thiết lập tài khoản từ thành phần gửi yêu cầu ràng buộc với FE11 được gửi đồng bộ bằng siêu dữ liệu nguồn an toàn và không lưu vào cơ sở dữ liệu thông tin xác thực/nội dung thiết lập | FR-FE10-010 | BR-FE10-002, BR-FE10-004 đến BR-FE10-008, BR-FE10-010 đến BR-FE10-013 | FT52, FT55 | FE10-S06 đến FE10-S08 | Hoàn thành |
| AC-FE10-011 | Danh sách cá nhân đã xác thực chỉ trả về DTO an toàn không nhạy cảm đủ điều kiện thuộc sở hữu, với bộ lọc SQL, thứ tự mới nhất trước và phân trang | FR-FE10-011 | BR-FE10-014, BR-FE10-015, BR-FE10-020 | Bản cập nhật cơ sở dữ liệu, tầng truy cập dữ liệu, tuyến, tích hợp và trình duyệt | FE10-I01 đến FE10-I03, FE10-I06 đến FE10-I08 | Hoàn thành |
| AC-FE10-012 | Số lượng chưa đọc chỉ gồm các bản ghi đủ điều kiện chưa đọc của người dùng đã xác thực | FR-FE10-012 | BR-FE10-014 đến BR-FE10-016, BR-FE10-020 | Tầng truy cập dữ liệu, tuyến, frontend, tích hợp và trình duyệt | FE10-I01 đến FE10-I04, FE10-I07, FE10-I08 | Hoàn thành |
| AC-FE10-013 | Đánh dấu một mục chỉ áp dụng cho bản ghi của chính mình, an toàn với dữ liệu nhạy cảm, có khả năng xử lý lặp an toàn và độc lập với việc gửi email | FR-FE10-013 | BR-FE10-014 đến BR-FE10-016 | Tầng truy cập dữ liệu, tuyến, frontend, tích hợp và trình duyệt | FE10-I01 đến FE10-I04, FE10-I06 đến FE10-I08 | Hoàn thành |
| AC-FE10-014 | Đánh dấu tất cả chỉ thay đổi các bản ghi đủ điều kiện chưa đọc của người dùng hiện tại bằng một dấu thời gian; gọi lại thao tác trả về 0 | FR-FE10-014 | BR-FE10-014 đến BR-FE10-016 | Bản cập nhật cơ sở dữ liệu, tầng truy cập dữ liệu, tuyến, frontend và trình duyệt | FE10-I01 đến FE10-I03, FE10-I06 đến FE10-I08 | Hoàn thành |
| AC-FE10-015 | Các mục đủ điều kiện chỉ nhận đường dẫn thao tác tương đối nằm trong danh sách cho phép của backend | FR-FE10-015 | BR-FE10-017 | Dữ liệu hiển thị, tuyến, frontend, tích hợp và trình duyệt | FE10-I02, FE10-I03, FE10-I05, FE10-I07, FE10-I08 | Hoàn thành |
| AC-FE10-016 | Chuông, phần xem trước, bộ lọc/phân trang hộp thư, thao tác đọc, trạng thái an toàn và điều hướng không chặn khớp với hợp đồng API | FR-FE10-016 | BR-FE10-014 đến BR-FE10-018 | Hợp đồng frontend, khung ứng dụng, trang và trình duyệt | FE10-I04 đến FE10-I08 | Hoàn thành |

### Tóm tắt độ bao phủ

- Tổng AC: 16 (AC-FE10-001 đến AC-FE10-016) - tất cả đều được ánh xạ; AC-FE10-011 đến AC-FE10-016 đã được triển khai, kiểm thử, H2/H3, CI/Azure đúng commit, hợp nhất và CI/Azure hậu hợp nhất xác nhận qua PR #75.
...
| -------------- | ----------- | ------ |
| BR-FE10-009 | Quét nguồn/cấu hình thông tin xác thực của nhà cung cấp | Hoàn thành |
| FR-FE10-003 | Yêu cầu kết quả tư cách thành viên FE04 và đặt chỗ sẵn sàng FE08 tạo một bản ghi đang chờ mà không thay đổi kết quả nguồn | Hoàn thành |
| FR-FE10-006 | Việc nhà cung cấp chấp nhận đặt `SENT`, `sentAt` và một lần thử thành công | Hoàn thành |
| BR-FE10-011 / Q-FE10-009 | Quyền sở hữu kết quả thành viên được ràng buộc bởi FE04 và ranh giới HTTP được bảo vệ | Hoàn thành |
| BR-FE10-010 / FR-FE10-005 / FR-FE10-009 | `notificationRoutes.test.js` từ chối ba lớp định nghĩa đã lưu không an toàn trước khi tạo nội dung từ mẫu, lưu vào cơ sở dữ liệu hoặc gọi nhà cung cấp; các giá trị được chèn khi chạy vẫn được làm sạch | Hoàn thành |
| BR-FE10-014 đến BR-FE10-020 / FR-FE10-011 đến FR-FE10-016 | Quyền sở hữu hộp thư cá nhân, loại trừ dữ liệu nhạy cảm, dữ liệu hiển thị an toàn, trạng thái đọc, danh sách thao tác cho phép, bản cập nhật cơ sở dữ liệu/cập nhật dữ liệu lịch sử và chấp nhận giao diện | Hoàn thành |
...
- [x] Chiến lược dùng bộ điều hợp nhà cung cấp đã cấu hình và đối tượng mô phỏng được truyền vào trong kiểm thử đã được xác nhận.
...
- [x] Không lưu/ghi nhật ký bí mật, thông tin xác thực của nhà cung cấp, mã thông báo/OTP thô hoặc nội dung xác thực nhạy cảm đã tạo từ mẫu.
...
- [x] Các cặp loại/mẫu đã quy định là chính xác và cờ của người gọi không thể bỏ qua chúng.
- [x] Thông báo xác thực nhạy cảm được gửi đồng bộ và nội dung nhạy cảm đã tạo từ mẫu chỉ tồn tại trong bộ nhớ của nhà cung cấp.
...
- [x] Các thao tác tạo/yêu cầu trùng/xử lý/thử lại chỉ dùng DTO tối thiểu.
- [x] `sourceEntityId` chỉ có số nguyên.
- [x] Danh sách cho phép của thành phần gửi yêu cầu nguồn đã ràng buộc và ranh giới HTTP `LIBRARIAN`/`ADMIN` được nêu rõ.
- [x] Khả năng xử lý lặp an toàn áp dụng xuyên suốt mọi trạng thái.
- [x] Thử lại thủ công sẽ giữ lại lịch sử không nhạy cảm và thử lại nhạy cảm sẽ trả về `REISSUE_REQUIRED`.
- [x] Tích hợp thành phần gửi yêu cầu OTP xác minh/đặt lại FE02 được phê duyệt qua ADR-004; `CHANGE_PASSWORD_OTP` và việc tích hợp người gọi FE09 vẫn được trì hoãn rõ ràng.
- [x] Nhân viên qua HTTP bị từ chối với mọi loại xác thực nhạy cảm; thành phần gửi yêu cầu không sở hữu bị từ chối với xác minh/đặt lại FE02 và thiết lập tài khoản FE11.
...
- [x] Bảo toàn cặp loại/mẫu đã quy định, khóa có độ nhạy tương đương bí mật, dữ liệu an toàn, DTO tối thiểu và các quy tắc sở hữu nguồn.
- [x] Yêu cầu từ chối an toàn trước khi tạo nội dung từ mẫu, lưu vào cơ sở dữ liệu thông báo/lần thử hoặc gửi qua nhà cung cấp.
...
- [x] Các trường DTO an toàn, `401`/`400`/`404` không thể phân biệt, thao tác lũy
  đẳng, thăm dò, phương án dự phòng khi lỗi, bản cập nhật cơ sở dữ liệu, triển khai và nghĩa vụ
...
- [x] Mã sản phẩm hộp thư, lược đồ chỉ bổ sung, API công khai, triển khai Azure,
  H2/H3, hợp nhất và các cổng hậu hợp nhất đã hoàn tất qua PR #75; không có yêu cầu
  hay hành vi mới được thêm bởi hoàn tất tài liệu này.
...
- BR-FE10-021: Quyền sở hữu `GENERAL_SYSTEM` phải được kiểm tra theo từng khóa
  mẫu đã quy định, không chỉ theo loại thông báo.
- BR-FE10-022: Các bản ghi hộp thư về kết quả mượn chỉ ánh xạ tới đường dẫn tương đối cố định
  `/borrowing/history`.
- BR-FE10-023: Dữ liệu gửi thông báo kết quả mượn không chứa lý do từ chối,
  email, mã thông báo, OTP, dấu vết ngăn xếp hoặc chi tiết nhà cung cấp.

### 18.2 Yêu cầu chức năng

- FR-FE10-017: Chấp nhận bốn mẫu FE07:
  `BORROW_REQUEST_APPROVED`, `BORROW_REQUEST_REJECTED`, `BORROW_RENEWED`,
  `BORROW_RETURNED`.
- FR-FE10-018: Mỗi khóa nguồn chỉ được lưu và gửi đúng một thông báo; việc gửi
  lại cùng yêu cầu không tạo bản ghi hoặc lần gửi trùng.
- FR-FE10-019: Bốn mẫu trên chỉ được hiển thị trong hộp thư cá nhân của đúng
  người nhận.
- FR-FE10-020: Backend trả đường dẫn thao tác cố định `/borrowing/history`; bên
  gọi không được cung cấp URL.

### 18.3 Tiêu chí chấp nhận và truy vết

- AC-FE10-017: Phê duyệt hoặc từ chối FE07 tạo đúng một hàng trong hộp thư, ánh xạ `AT-002`,
  `AT-003`.
- AC-FE10-018: Gửi lại cùng khóa nguồn phải trả bản ghi hiện có, không tạo bản sao.
- AC-FE10-019: Khi nhấn thông báo, hệ thống cố gắng đánh dấu đã đọc rồi mở đường
  dẫn cố định; việc đánh dấu thất bại không được chặn điều hướng, và đường dẫn
  được ánh
  xạ `AT-006`.
- AC-FE10-020: FE10 thất bại không hoàn tác FE07/FE08 và dữ liệu không lộ dữ
