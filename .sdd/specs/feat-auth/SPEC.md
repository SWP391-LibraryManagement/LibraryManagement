...
- EC-FE02-019: Khi API CAPTCHA không tải được lúc khởi tạo, giao diện thử lại đúng một lần rồi giữ nút gửi đăng nhập/đăng ký ở trạng thái vô hiệu hóa nếu vẫn không có challenge. Khi yêu cầu đổi mã thủ công thất bại, challenge còn dùng được không bị xóa; dữ liệu biểu mẫu luôn được giữ nguyên và người dùng vẫn có thể yêu cầu challenge mới.
...
| FR-FE02-030 | Từ chối CAPTCHA thiếu, hết hạn, không tồn tại, đã dùng, replay hoặc sai trước khi dispatch service xác thực; phục hồi tải có giới hạn mà không xóa challenge còn dùng được | EC-FE02-019 | BR-FE02-029 | `backend/tests/captchaService.test.js`; `backend/tests/captchaRoutes.test.js`; `frontend/test/captchaFrontend.test.js`; `frontend/test/captchaRecovery.test.js` | FE02-T071 H2 APPROVED - PENDING EXACT-HEAD CI/H3 |
...
- PR #111 đã merge thành `1f0905f` sau exact-head CI và H3. FE02-T071 sửa lỗi reliability phát hiện sau merge; RED-GREEN, full local gate và H2 đã đạt, nhưng vẫn cần exact-head CI, H3 và staging verification mới trước khi đóng.
