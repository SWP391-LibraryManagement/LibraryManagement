...
- Smoke test gọi `GET /api/auth/captcha`, chỉ chấp nhận đúng ba trường công khai `image`, `captchaToken`, `expiresIn` và fail-closed nếu route thiếu, giá trị sai hoặc có bất kỳ trường bổ sung nào.
- Không đổi token, TTL, capacity, storage, API shape, dependency hoặc luồng xác thực ngoài CAPTCHA.

Trạng thái: H3 REMEDIATION ROUND 2 H2 APPROVED - PENDING EXACT-HEAD CI/H3
Ngày: 2026-08-04
Chủ sở hữu: Dat
