...
Tạo đặt chỗ cho bản sao vật lý, hiển thị cho Thành viên và Thủ thư/Quản trị viên, xử lý hàng đợi cho kết quả ổn định, hủy, hết hạn giữ chỗ, bàn giao hoàn tất FE07, lỗi thông báo FE10, phân quyền, phân trang, thứ tự, toàn vẹn giao dịch và truy vết kiểm toán.

Các tác vụ chuẩn hóa vòng đời và danh sách bản sao có thể đặt chỗ đã hoàn tất. Bằng chứng tự động bao phủ FE08-T028 đến FE08-T039; H3, hợp nhất, CI và Azure trên đúng commit đã hoàn tất qua PR #89.
...
- Đang mượn cùng đầu sách: trạng thái `BORROWED` của FE07 loại mọi bản sao có cùng `BookId` khỏi danh sách có thể đặt chỗ; yêu cầu tạo trực tiếp trả về `BOOK_ALREADY_BORROWED`; lịch sử mượn đã kết thúc không chặn và lượt cũ trong hàng đợi bị bỏ qua mà không thay đổi dữ liệu.
...
- `GET /api/reservations`: chỉ Thủ thư/Quản trị viên được truy cập; từ chối Thành viên; kiểm tra bộ lọc, thứ tự `ReservedAt ASC, ReservationId ASC`, giá trị mặc định và việc từ chối page/limit không hợp lệ.
- `POST /api/reservations/process-queue`: chỉ Thủ thư/Quản trị viên được truy cập; bắt buộc có `copyId`, từ chối `bookId`; bao phủ lượt đặt chỗ được chọn, hàng đợi rỗng, bỏ qua lượt không đủ điều kiện, lỗi thông báo và chọn đồng thời.
- `POST /api/reservations/expire-holds`: cho hết hiệu lực các lượt giữ quá hạn, chuyển lượt cho Thành viên đủ điều kiện tiếp theo và giữ nguyên trạng thái khi không có lượt giữ nào quá hạn.
...
- Tích hợp FE07: hoàn tất lượt đặt chỗ của đúng Thành viên, từ chối Thành viên khác mượn, giữ ưu tiên hàng đợi đang hoạt động, từ chối gia hạn và không tiết lộ chủ sở hữu lượt đặt chỗ.
- Danh sách bản sao có thể đặt chỗ: `GET /api/reservations/candidates` chỉ dành cho Thành viên; lọc các bản sao `BORROWED`/`RESERVED` của sách đang hoạt động; loại đầu sách có cùng `BookId` mà Thành viên đang mượn; chỉ trả bảy trường an toàn, trong đó `hasActiveReservation` theo phạm vi Thành viên; tìm kiếm/phân trang trên máy chủ; thứ tự ổn định; đếm lượt đang hoạt động; vô hiệu hóa thao tác trùng và giữ `POST /api/reservations { copyId }` là thao tác tạo chính thức.

## 4. Luồng chấp nhận E2E / thủ công

- Thành viên đủ điều kiện đặt chỗ một bản sao chưa khả dụng -> Thủ thư/Quản trị viên xử lý hàng đợi -> yêu cầu thông báo FE10 được tạo -> chính Thành viên đó mượn bản sao đang được giữ -> lượt đặt chỗ chuyển thành `FULFILLED` và bản sao chuyển thành `BORROWED` trong cùng giao dịch.
...
- Thủ thư/Quản trị viên mở danh sách mà không truyền tham số phân trang -> 20 bản ghi đầu hiển thị theo thứ tự ổn định; giới hạn không hợp lệ trả về lỗi xác thực.
- Thành viên tìm danh sách bản sao có thể đặt chỗ -> máy chủ trả các bản ghi an toàn, kể cả sách đã được chính Thành viên đó đặt chỗ; Thành viên tạo lượt đặt chỗ thật bằng `copyId` dạng số, thao tác trùng bị vô hiệu hóa và danh sách đặt chỗ chính thức được tải lại.
- Mảng vai trò cũ/không hợp lệ chứa `MEMBER + LIBRARIAN` hoặc `MEMBER + ADMIN` bị chuyển hướng khỏi màn hình tự phục vụ của Thành viên và nhận `403 ROLE_REQUIRED` từ các endpoint danh sách bản sao có thể đặt chỗ/tạo/danh sách riêng/hủy; tài khoản được lưu vẫn có một vai trò và thao tác hàng đợi của Thủ thư/Quản trị viên vẫn dùng được.
- Thành viên chọn một sách chưa khả dụng trên FE01 -> `/reservations/mine?bookId=...` phân giải tiêu đề công khai -> danh sách FE08 được bảo vệ hiển thị các bản sao vật lý phù hợp -> Thành viên chọn `copyId` thật.
- Thành viên có một lượt đã hủy và một lượt `ACTIVE`/`NOTIFIED` mới cho cùng bản sao -> trạng thái hiện tại xuất hiện ở phần đang hoạt động với nhãn và thao tác đúng bản sao -> trạng thái đã hủy chỉ còn trong lịch sử.
...
- Thành viên đang mượn một bản sao vật lý -> mọi bản sao cùng `BookId` bị loại khỏi danh sách có thể đặt chỗ -> tạo trực tiếp cho bản sao khác trả `409 BOOK_ALREADY_BORROWED` -> khi Thủ thư/Quản trị viên xử lý, lượt cũ trong hàng đợi bị bỏ qua và vẫn giữ trạng thái `ACTIVE`.
...
- `frontend/test/reservationFrontend.test.js` bao phủ vòng đời, cô lập lỗi, tải lại dữ liệu máy chủ và hợp đồng API/trang chọn bản sao có thể đặt chỗ.
...
- Bằng chứng CI lịch sử: commit `236043864304627f3577baafa9b8648c13c7a691` nằm trong `main`; GitHub Actions run `29217437981` đã chạy thành công.
...
- Lệnh SQL cho danh sách bản sao có thể đặt chỗ đạt `2/2` kiểm thử hợp đồng nguồn và bỏ qua `2` ca có thay đổi SQL vì chưa cấu hình cơ sở dữ liệu dùng một lần đã được phê duyệt.
- FE08-T028 đến FE08-T034 đạt cổng backend/ranh giới dùng chung tập trung 77/77 và frontend 9/9; truy vết là 29/29.
- FE08-T035 đến FE08-T039 đạt: hợp đồng backend cho danh sách bản sao có thể đặt chỗ 23/23, SQL 2/2, toàn bộ frontend tại thời điểm ghi nhận 149/149 cùng lint/build, kiểm thử trình duyệt tập trung 1/1 và toàn bộ Playwright 4/4 trên các cổng cô lập.
...
- `TD-028` đã được giải quyết trong phạm vi triển khai và xác thực tự động; trang Thành viên hiện dùng danh sách bản sao có thể đặt chỗ được bảo vệ, có SQL hỗ trợ và không còn import `DEMO_RESERVABLE`.
...
| NFR-FE08-TXN-001 | Giữ chỗ trong hàng đợi và hoàn tất FE07 trong cùng giao dịch, chỉ có một bên thắng khi xử lý đồng thời. | Bộ kiểm thử SQL cho nghiệp vụ mượn trên cơ sở dữ liệu dùng một lần và kiểm thử tích hợp FE07/FE08 đều đạt. |
...
| NFR-FE08-PERF-001 | Giá trị mặc định và giới hạn phân trang; giá trị không hợp lệ bị từ chối trước khi gọi tầng truy cập dữ liệu. | Kiểm thử validator/tầng truy cập dữ liệu của API danh sách bản sao có thể đặt chỗ và FE08-T028 đạt. |
...
| NFR-FE08-SEC-004 | API đọc danh sách bản sao có thể đặt chỗ yêu cầu đúng vai trò `MEMBER`, từ chối tài khoản Quản trị viên/Thủ thư và không công khai siêu dữ liệu bản sao hoặc thông tin chủ sở hữu chỉ dành cho nhân viên. | Bao phủ tuyến API/frontend FE08-T035/T036 cùng FE08-T041 đạt. |
| NFR-FE08-PERF-003 | API đọc danh sách bản sao có thể đặt chỗ dùng phân trang có giới hạn trên máy chủ và thứ tự tiêu đề/sách/bản sao ổn định. | Bằng chứng SQL, backend và frontend FE08-T035/T036/T037 đạt. |
