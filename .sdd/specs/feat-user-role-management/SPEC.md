...
>
> The admin-created account setup revision is approved through `ADR-005-admin-created-account-setup-boundary.md`; this SPEC revision still requires the normal human review gate before implementation.
...
5. In one transaction, the system creates a new user record with status `INACTIVE`, assigns the Member role, stores an unusable bcrypt hash of a discarded server-generated value, creates a hashed `ACCOUNT_SETUP` token with a 24-hour expiry, and writes the FE11 audit entry.
...
2. Admin edits one or more fields: full name, phone, address, or email, and submits the `expectedUpdatedAt` value from the loaded record.
3. The system validates updated email if changed (must be unique).
4. The system updates the record only when the stored `UpdatedAt` still equals `expectedUpdatedAt`.
...
5. The system sets user status to `INACTIVE` and records server timestamp `deactivatedAt`.
6. The system keeps user data intact (does not delete).
7. In the same transaction, the system invalidates all active refresh/session credentials for that user.
8. The system writes the audit log entry in the same transaction; if credential invalidation or audit persistence fails, the deactivation rolls back.
...
4. The system validates email format and checks for duplicate email.
5. In one transaction, the system creates a new user record with status `INACTIVE`, assigns the Librarian role, stores an unusable bcrypt hash of a discarded server-generated value, creates a hashed `ACCOUNT_SETUP` token with a 24-hour expiry, and writes the FE11 audit entry.
6. After the source transaction commits, FE11 requests `ACCOUNT_SETUP` delivery through the requester bound to `FE11`, using `AuthToken` source metadata and idempotency key `FE11:ACCOUNT_SETUP:<tokenId>`.
7. FE10 renders and sends the setup link without persisting or returning the raw token, link, or rendered sensitive content.
8. Password-based login remains unavailable while the account is `INACTIVE`.
9. If delivery fails, the account remains `INACTIVE` and the response reports only safe delivery status; the Admin can use the approved resend flow.
...
2. Admin edits the librarian fields `fullName`, `phone`, `email`, `department`, and `specialization`.
...
3. The system sets librarian user status to `INACTIVE` and records server timestamp `deactivatedAt`.
4. In the same transaction, the system invalidates all active refresh/session credentials for that librarian.
5. The system writes the audit log entry in the same transaction; failure rolls back the deactivation.
...
3. The system confirms that the target account is `INACTIVE`, has `ACCOUNT_SETUP` token history, has not completed setup, and is outside the 60-second issuance cooldown.
4. In one transaction, FE11 revokes prior active setup tokens, creates a new hashed `ACCOUNT_SETUP` token with a 24-hour expiry, and writes the resend audit entry.
...
- BR-FE11-024: User, profile, initial role, setup token, and FE11 audit creation must commit or roll back together; FE10 provider delivery occurs after this source transaction and remains non-blocking.
- BR-FE11-025: Admin resend is allowed only for an `INACTIVE` admin-created account with incomplete setup token history; each resend revokes prior active setup tokens and creates a new token/event/key after a 60-second cooldown.
...
- BR-FE11-027: Every user-information update must use optimistic concurrency with the loaded `UpdatedAt` value; a stale update returns HTTP `409` with code `STALE_USER_STATE` and persists no field or audit-success change. If the submitted values produce no effective field change, the system returns the current safe DTO with HTTP `200`, does not advance `UpdatedAt`, and writes no success audit.
...
- FR-FE11-003: When Admin creates a new user account with valid data, the system shall atomically create an `INACTIVE` user, profile, approved role, hashed setup token, and audit entry, then request one FE10 setup delivery and return safe delivery status.
...
- FR-FE11-008: When admin deactivates a user account, the system shall atomically set status to `INACTIVE`, set `deactivatedAt`, invalidate all active refresh/session credentials, and write the audit record.
- FR-FE11-009: When Admin creates a new librarian account with valid data, the system shall atomically create an `INACTIVE` user, profile, Librarian role, hashed setup token, and audit entry, then request one FE10 setup delivery and return safe delivery status.
- FR-FE11-010: When admin updates librarian account information, the system shall validate and save `department` and `specialization` when those fields are submitted.
- FR-FE11-011: When admin deactivates a librarian account, the system shall atomically set status to `INACTIVE`, set `deactivatedAt`, invalidate all active refresh/session credentials, and write the audit record.
...
- FR-FE11-036: When Admin requests setup resend for an eligible incomplete account after cooldown, FE11 shall revoke prior active setup tokens, create a new token ID, write an audit entry, and request one new FE10 `ACCOUNT_SETUP` delivery.
...
- FR-FE11-023: WHERE an update's `expectedUpdatedAt` does not equal the current stored `UpdatedAt`, the system shall reject the update with HTTP `409` and code `STALE_USER_STATE`, preserving the existing record and writing no success audit. (Source: EC-FE11-007, BR-FE11-027)
...
- AC-FE11-004: Given an existing user and a matching `expectedUpdatedAt`, when Admin submits at least one effective change, then the allowed changes are saved and `UpdatedAt` advances; when the submission is a no-op, then the current safe DTO is returned with unchanged `UpdatedAt` and no success audit.
...
- AC-FE11-023: Given an Admin submits an update with stale `expectedUpdatedAt`, when the current user record has changed, then the system returns `409 STALE_USER_STATE` and persists no submitted field or success audit.
...
| EC-FE11-007 | Concurrent updates to same user | Require `expectedUpdatedAt`; reject a mismatch with `409 STALE_USER_STATE`, persist no submitted field, and write no success audit. |
...
| `createdAt`, `updatedAt`, `lastLoginAt` | Return as server-generated timestamps; `updatedAt` is also the optimistic-concurrency value. |
...
| POST | `/api/users` | Admin | `{ email: string, fullName: string, type: "member"\|"librarian", phone?: string, address?: string, department?: string, specialization?: string }` | `201 { userId, email, status: "INACTIVE", roles, setupDeliveryStatus, message }` | Creates source state atomically, then requests FE10 delivery; never returns password/token/link. |
| PUT | `/api/users/{userId}` | Admin | `{ expectedUpdatedAt: datetime, fullName?: string, phone?: string, address?: string, email?: string, department?: string, specialization?: string }` | Updated `UserManagementView`; stale state: `409 { code: "STALE_USER_STATE" }` | Uses optimistic concurrency; a stale request changes nothing. |
| PATCH | `/api/users/{userId}/status` | Admin | `{ status: "INACTIVE", expectedUpdatedAt: datetime }` | Updated user status | Deactivation sets `deactivatedAt`, invalidates credentials, and uses optimistic concurrency. |
...
| POST | `/api/users/{userId}/resend-setup` | Admin | `{}` | `200 { userId, status: "INACTIVE", setupDeliveryStatus, message }` | Eligible incomplete account only; revokes prior active token and enforces 60-second cooldown. |
...
- NFR-FE11-TXN-001: Creating a user must be atomic: user record, profile, default role, hashed setup token, and audit log must succeed together or roll back.
...
- NFR-FE11-TXN-005: Setup resend token revocation, new token creation, and audit logging must commit or roll back together.
