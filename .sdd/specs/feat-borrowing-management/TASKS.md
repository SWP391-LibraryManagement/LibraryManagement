...
v0.9.0 được H3 phê duyệt ở commit `08e472f` và tích hợp vào `main` qua
`ba29dc0`. Bản hoàn tất `6189b1a` sau đó được phê duyệt H3 trong nhiệm vụ,
...
- [x] FE07-T01 Thêm tuyến API mượn từ hợp đồng API đã phê duyệt.
- [x] FE07-T02 Thêm validator cho ID yêu cầu, ID chi tiết, ID bản sao, trạng thái, ngày, lý do từ chối, tình trạng trả và ghi chú.
- [x] FE07-T03 Thêm quy tắc tầng dịch vụ mượn cho điều kiện hợp lệ thành viên, giới hạn mượn, phạt chưa thanh toán, khoản mượn quá hạn, khả dụng bản sao và mục yêu cầu trùng lặp.
- [x] FE07-T04 Thêm phương thức tầng truy cập dữ liệu SQL cho tạo, liệt kê, phê duyệt, từ chối, trả và gia hạn yêu cầu mượn.
...
- [x] FE07-T26 Thêm khả năng tiếp cận: caption bảng, phạm vi tiêu đề cột, nhãn biểu mẫu, hỗ trợ bàn phím.
...
- [x] **FE07-T031 - Thêm kiểm thử RED cho điều kiện hợp lệ chuẩn và sách cha.**
...
- [x] **FE07-T035 - Đối soát chính sách ngày Thành phố Hồ Chí Minh và lý do từ chối.**
  ...
  - GREEN: suy ra ngày nghiệp vụ trong `Asia/Ho_Chi_Minh`, xác thực phạm vi trả bao gồm hai đầu và lưu lý do từ chối đã cắt khoảng trắng trong giao dịch kiểm toán từ chối.
  - Xác minh: kiểm thử tuyến API/SQL tập trung đạt và lệnh không hợp lệ giữ nguyên trạng thái yêu cầu/chi tiết/bản sao/kiểm toán.
...
- [x] **FE07-T036 - Căn chỉnh schema, mô hình, OpenAPI và siêu dữ liệu truy vết.**
  ...
  - RED: kiểm thử hợp đồng/mô hình xác nhận trường người tạo/người phê duyệt/ngày mượn bắt buộc, ngày trước phê duyệt có thể để trống, enum trạng thái đã lưu, dữ liệu đầu vào lý do, mô tả từ chối ngày tương lai và chỉ `OVERDUE` dẫn xuất.
...
- [x] **FE07-T038 - Chạy cổng xác thực tập trung và rà soát v0.5.0.**
  ...
  - Xác minh: kiểm thử tuyến API/tầng truy cập dữ liệu/hợp đồng/mô hình tập trung, kiểm thử đồng thời SQL, kiểm thử frontend, `npm.cmd run trace:enforce` và `git diff --check` đạt; bộ đầy đủ chỉ chạy tại cổng hợp nhất.
...
- [x] B6 L2.3 căn chỉnh các trạng thái FE07 đã lưu, siêu dữ liệu SQL và hợp đồng OpenAPI với khi chạy/đặc tả đã phê duyệt.
- [x] B6 L2.3 thêm kiểm thử mô hình/OpenAPI tập trung và bao phủ bộ lọc mượn thành viên trực tiếp của nhân sự.
- [x] B6 L3 củng cố xử lý đọc lại sau commit, điều kiện hợp lệ phê duyệt trong thời gian giao dịch, tính nguyên tử kiểm toán từ chối/gia hạn và xác thực ngày dương lịch nghiêm ngặt.
- [x] B6 L3 thêm hồi quy trong bộ nhớ và bằng chứng SQL trực tiếp cho điều kiện hợp lệ phê duyệt và hoàn tác kiểm toán từ chối/gia hạn.
- [x] B6 L4 loại bỏ trạng thái trình duyệt FE07 giả, thêm bảo vệ tuyến API hoàn chỉnh, sửa hành vi hộp thoại/đáp ứng và xác thực quy trình thành viên/nhân sự thực.
- [x] Khắc phục rà soát độc lập B6: thành viên không rõ `404`, race từ chối `409`, ngày trả do máy chủ sở hữu, UI phê duyệt/trả trung thực, phân vùng khoản mượn đang chờ, tiêu điểm hộp thoại và phân trang di động.
...
- [x] Nhat xác nhận rà soát của con người vào 2026-07-14 trước commit, push hoặc hợp nhất.

### 4.1 Xác thực v0.5.0 đã đóng

- [x] Kiểm thử tuyến API/tầng truy cập dữ liệu/hợp đồng/mô hình mượn tập trung đạt: 66/66.
...
| FR-FE07-022 | borrowingConcurrency.sqltest.js > Kiểm thử SQL hoàn tác kiểm toán thất bại cho tạo/phê duyệt/trả/từ chối/gia hạn |
...
- [x] **FE07-T041 - Hiển thị đúng yêu cầu mượn bị từ chối trong lịch sử thành viên.**
  ...
  - Xác minh: kiểm thử tuyến API/hợp đồng backend tập trung, kiểm thử mượn frontend, lint/build, truy vết và vệ sinh diff đạt.
  - Bằng chứng: backend tập trung 61/61, frontend FE07 21/21, backend đầy đủ 925/925, frontend đầy đủ 199/199, lint frontend, build môi trường triển khai thực tế, truy vết FE07 29/29 và `git diff --check` đã đạt vào 2026-07-22.
...
- [x] PR #19 đã hợp nhất vào `main` thành `aeed0dfecb764e6cbe63d7074727f318700e59ea`.
- [x] Lượt chạy GitHub Actions CI `29308540692` đã đạt cho commit hợp nhất và bao phủ truy vết, kiểm thử backend, lint/kiểm thử/build frontend và import health backend.
...
## 2026-07-22 - Đợt hiệu chỉnh

- [x] Ổn định bố cục Xử lý trả trong khi giữ lệnh trả và hành vi chọn chuẩn.
- [x] Làm cho từng hành động hàng `Xử lý trả` mở xác nhận trả chuẩn cho chính xác khoản mượn đó.
- [x] Gắn hộp thoại phê duyệt/từ chối với mục tiêu yêu cầu rõ ràng và thêm tương thích schema BorrowRequests đã triển khai.
- [x] **FE07-T042 - Bổ sung quyết định nhân sự và ổn định dữ liệu đầu vào từ chối.**
  ...
  - RED: yêu cầu cả hai hộp thoại quyết định hiển thị ngữ cảnh yêu cầu/thành viên/mọi bản sao chuẩn và yêu cầu quản lý tiêu điểm hộp thoại dùng chung tồn tại qua các lần trường nhập liệu được kiểm soát render lại.
  ...
  - Theo dõi: loại bỏ thông báo khả dụng chung dư thừa và trường mô hình hiển thị tổng hợp không dùng; các trạng thái bản sao chuẩn riêng vẫn hiển thị và máy chủ vẫn xác thực lại khi phê duyệt.
  - Ranh giới: không đổi API/schema/vai trò/giao dịch; Thủ thư/Quản trị viên giữ các lệnh FE07 chuẩn như cũ.
- [x] **FE07-T043 - Xác minh và rà soát con người hiệu chỉnh v0.7.3.**
  ...
  - Cổng cuối đã đóng qua các lượt H2/H3 và hoàn tất PR #89.
...
- [x] **FE07-T046 - Làm kiểm tra thời gian nghiệp vụ và trạng thái trả có tính xác định.**
  ...
  - Xác minh: kiểm thử FE07 tập trung đạt dưới `TZ=UTC`; SQL dùng một lần và hồi quy đầy đủ đạt. H2 ban đầu và phụ lục H2 đã đạt; commit `97aca62` và lượt chạy PR CI `30014066260` đã đạt. H2 mới sau đó phê duyệt commit khắc phục `b931e00`, và lượt chạy PR CI `30019439505` đã đạt. Phát hiện H3 vòng hai đã được khắc phục, rà soát lại và tích hợp; hoàn tất cuối được xác nhận qua PR #89, CI và Azure trên đúng commit.
...
- [x] **FE07-T052 - Loại bỏ kịch bản gia hạn đa vai trò đã bị thay thế.**
  ...
  - Đối soát: giữ gia hạn chỉ chủ sở hữu Thành viên và gia hạn liên thành viên của Thủ thư/Quản trị viên bằng tác nhân một vai trò riêng; loại kiểm thử đa vai trò cục bộ nhánh và phần chênh lệch ủy quyền không cần thiết.
  - Tệp: `backend/tests/borrowingRoutes.test.js`, `backend/src/services/borrowingService.js`.
  - Bằng chứng: chọn bất biến một vai trò/FE11 đạt 3/3 trước dọn dẹp và 3/3 sau dọn dẹp; xác minh tuyến API/tầng truy cập dữ liệu FE07 đầy đủ đạt 79/79.
- [x] **FE07-T049 - Trả về ảnh chụp trạng thái giao dịch chính thức.**
  - Ánh xạ tới: BD-002, BR-FE07-014/016, FR-FE07-007/008, AC-FE07-008.
  - RED: thay đổi hạn trả giữa kiểm tra trước và khóa tầng truy cập dữ liệu tạo dữ liệu quá hạn phản hồi/kiểm toán cũ.
  - GREEN: tầng truy cập dữ liệu trả SQL và trong bộ nhớ xây bằng chứng kiểm toán từ hạn trả đã khóa và ngày trả đã commit; tầng dịch vụ xây `fineCandidate` từ cùng bằng chứng đó.
  ...
  - Bằng chứng: RED trả 12 thay vì 2 ngày quá hạn và thất bại hợp đồng nguồn tầng truy cập dữ liệu; GREEN FE07 tập trung và đầy đủ đạt. SQL thay đổi không chạy vì không cấu hình DB dùng một lần có tên hoặc cờ thay đổi.
- [x] **FE07-T050 - Dùng phép tính lịch gia hạn dùng chung.**
  ...
  - GREEN: phần mở rộng tầng dịch vụ và mọi so sánh gia hạn-quá hạn dùng helper `libraryBusinessTime`; không còn `setDate`, `getDate` hoặc `setHours` cục bộ máy chủ trong đường gia hạn bị ảnh hưởng.
...
- [x] **FE07-T051 - Hoàn tất xác minh và bằng chứng FE07.**
  ...
  - Bằng chứng: kiểm thử tuyến API/tầng truy cập dữ liệu tập trung, ma trận múi giờ UTC/New York,
    hồi quy backend đầy đủ, truy vết, vệ sinh diff, rà soát L2/L3 và
    chấp nhận khi chạy cục bộ; SQL thay đổi tùy chọn được ghi riêng.
  - Bằng chứng tích hợp cuối với hợp nhất `e20fdc3` đang mở: mốc cơ sở một vai trò
    3/3; 7 bộ tập trung/281 kiểm thử; tuyến API/tầng truy cập dữ liệu FE07 79/79;
...
- [x] **FE07-T053 - Thực thi một yêu cầu đang chờ trên mỗi bản sao xuyên vai trò.**
  ...
  - Frontend: chi tiết Quản trị hiển thị trạng thái bản sao vật lý; lỗi quyết định
    của Quản trị và Thủ thư tải lại trạng thái chuẩn; trợ giúp từ chối giải thích
...
- [x] **FE07-T055 - Đưa danh mục lưu hành Quản trị vừa khung nhìn màn hình máy tính.**
  - Ánh xạ tới: FR-FE07-038, AC-FE07-031.
  - Loại các cột ID yêu cầu và mã vạch riêng khỏi danh mục lưu hành Quản trị và dữ liệu hiển thị danh mục DOCX mà không xóa trường API/cơ sở dữ liệu chuẩn nào.
  - Giữ ID chi tiết mượn làm mã định danh vận hành hiển thị, căn chỉnh mọi tiêu đề cột còn lại với giá trị hàng của nó và bọc nội dung thành viên/sách dài trong ô bố cục cố định.
...
- [x] **FE07-T056 - Kết nối lựa chọn trả quá hạn với tạo khoản phạt FE09.**
  ...
  - Chỉ hiển thị `Tạo phiếu phạt` cho khoản mượn đang hoạt động đã chọn có hạn trả chuẩn quá hạn trong `Asia/Ho_Chi_Minh`.
...
## 2026-07-29 - Đợt FE07-FE12 v0.9.0
...
- [x] **FE07-T060 - Tích hợp/E2E và bằng chứng.**
  ...
  - Bằng chứng H2 cục bộ: backend FE07 3 suite/90 kiểm thử; frontend
    ...
    1/1 tại 1440x900; toàn bộ 69 bộ kiểm thử backend với 1.125 ca kiểm thử và frontend
