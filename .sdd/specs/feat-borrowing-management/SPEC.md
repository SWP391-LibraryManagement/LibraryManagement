...
>
> Bản sửa đổi v0.7.4 yêu cầu các quyết định quá hạn khi trả/gia hạn phải dùng trình hỗ trợ ngày nghiệp vụ
...
> duy trì quy tắc một vai trò, nguồn dữ liệu chính thức và ngày nghiệp vụ.
>
...
> `BookCopies.Status`. Danh sách bản sao Thành viên có thể yêu cầu mượn và thao tác tạo đều loại trừ bản sao đã được yêu cầu;
...
> `BORROWED`). API liệt kê bản sao và giao dịch tạo đều thực thi quy tắc; dữ liệu cũ
...
Tính năng này là cốt lõi vì dữ liệu mượn sai có thể làm hỏng hàng tồn kho, tiền phạt, đặt chỗ, báo cáo và lịch sử kiểm tra.
...
4. Hệ thống xác nhận lại rằng cả chi tiết mượn và bản sao vật lý đều là `BORROWED`, sau đó lưu ngày trả theo ngày nghiệp vụ `Asia/Ho_Chi_Minh`.
...
7. Hệ thống tính số ngày quá hạn theo lịch giữa hạn trả và ngày nghiệp vụ trả sách trong `Asia/Ho_Chi_Minh`, rồi cung cấp dữ liệu trả quá hạn, hư hỏng hoặc thất lạc cho FE09 Quản lý tiền phạt.
...
3. Hệ thống kiểm tra điều kiện gia hạn của chủ khoản mượn theo ngày nghiệp vụ `Asia/Ho_Chi_Minh` hiện tại: chưa quá hạn, không có khoản phạt chưa thanh toán, số lần gia hạn bằng 0 và không có lượt đặt chỗ đang hoạt động của thành viên khác từ FE08.
4. Hệ thống kéo dài hạn trả thêm 14 ngày theo lịch kể từ hạn trả hiện tại bằng trình hỗ trợ ngày nghiệp vụ dùng chung.
...
5. Hệ thống hỗ trợ lọc theo trạng thái chi tiết và khoảng ngày nghiệp vụ bao gồm cả hai đầu.
...
2. Trước khi Thủ thư phê duyệt, một quy trình khác thay đổi trạng thái bản sao hoặc trạng thái đặt chỗ.
...
- BR-FE07-005A: Trạng thái phê duyệt FE04 xác định hạn mức mượn hằng ngày nhưng không tự chặn quyền mượn: `Members.Status = APPROVED` cho phép 5 bản sao trong mỗi ngày nghiệp vụ `Asia/Ho_Chi_Minh`; `NONE`, `PENDING`, `REJECTED` hoặc `INACTIVE` cho phép 3 bản sao trong mỗi ngày nghiệp vụ.
...
- BR-FE07-008: Phê duyệt phải kiểm tra lại khả năng mượn bản sao có xét trạng thái đặt chỗ và điều kiện của thành viên.
...
- BR-FE07-011: Mỗi lần trả sách phải lưu ngày trả theo múi giờ hoạt động của thư viện `Asia/Ho_Chi_Minh`; ngày này không thể trước `BorrowDate` hoặc sau ngày nghiệp vụ hiện tại. Thao tác trả yêu cầu cả chi tiết mượn và bản sao vật lý vẫn ở trạng thái `BORROWED`; trạng thái bản sao không nhất quán trả về `BORROW_STATE_CONFLICT` mà không thay đổi dữ liệu.
...
- BR-FE07-015: Mỗi chi tiết mượn được gia hạn tối đa 1 lần; một lần gia hạn hợp lệ sẽ kéo dài hạn trả hiện tại thêm 14 ngày theo lịch bằng trình hỗ trợ ngày nghiệp vụ `Asia/Ho_Chi_Minh` dùng chung và không bao giờ dùng phép tính `Date` theo múi giờ cục bộ của máy chủ.
...
- BR-FE07-024: Hàng đợi đặt chỗ `ACTIVE` của một bản sao chặn việc tạo và phê duyệt yêu cầu mượn thông thường cho đến khi nhân viên xử lý hoặc giải quyết hàng đợi đó.
...
- BR-FE07-031: Danh sách bản sao có thể yêu cầu mượn, API tạo yêu cầu và lịch sử cá nhân dành cho Thành viên yêu cầu tài khoản chỉ có vai trò `MEMBER`; tài khoản `LIBRARIAN` và `ADMIN` không thể tự đặt hoặc mượn sách.
- BR-FE07-032: Chi tiết `BorrowDetails.Status = BORROWED` hiện tại là tín hiệu chính thức của FE08 rằng cùng một Thành viên không thể đặt chỗ bất kỳ bản sao nào khác có cùng `BookId`; trạng thái chi tiết đã kết thúc không chặn việc đặt chỗ sau này.
...
- FR-FE07-009: Khi có yêu cầu gia hạn, tài khoản một vai trò `LIBRARIAN` hoặc `ADMIN` có thể xử lý khoản mượn của nhiều thành viên; tài khoản một vai trò `MEMBER` chỉ được xử lý chi tiết do mình sở hữu. Hệ thống phải kiểm tra mọi điều kiện chặn của chủ khoản mượn, chỉ cho phép tối đa 1 lần gia hạn và chỉ cộng thêm 14 ngày theo lịch bằng trình hỗ trợ ngày nghiệp vụ `Asia/Ho_Chi_Minh` dùng chung khi tất cả quy tắc đều đạt.
...
- FR-FE07-031: Khi Thủ thư/Quản trị viên xem khoản mượn đang hoạt động để xử lý trả, UI phải giữ nguyên ngày mượn, hạn trả và số lần gia hạn chính thức từ `BorrowDetails`; trạng thái hạn trả được tính theo ngày nghiệp vụ `Asia/Ho_Chi_Minh` hiện tại và hiển thị rõ `Còn N ngày`, `Đến hạn hôm nay` hoặc `Quá hạn N ngày`, không đặt nhãn `Đúng hạn` dưới tiêu đề `Quá hạn`.
...
- FR-FE07-033: KHI Thành viên tiếp tục từ lượt giữ FE08 `NOTIFIED` do mình sở hữu, giao diện FE07 phải chọn đúng `bookId` và `copyId` do danh sách bản sao có thể mượn được bảo vệ trả về, rồi gửi bản sao đó qua quy trình yêu cầu chờ xử lý thông thường; kiểm tra trạng thái đặt chỗ phía máy chủ vẫn là nguồn quyết định chính thức.
- FR-FE07-034: KHI Thành viên xem danh sách bản sao có thể yêu cầu mượn hoặc tạo yêu cầu, FE07 sẽ loại trừ/từ chối mọi bản sao đã thuộc một yêu cầu đang chờ khác; giao dịch tạo chính thức trả về `409 COPY_PENDING_REQUEST_CONFLICT` mà không ghi một phần yêu cầu/chi tiết/kiểm toán.
...
- FR-FE07-036: KHI Thành viên xem danh sách bản sao hoặc tạo yêu cầu, FE07 sẽ ẩn/từ chối mọi bản sao có `BookId` đã có quy trình đang hoạt động cho Thành viên đó; giao dịch tạo chính thức trả về `409 BOOK_ALREADY_IN_BORROWING_WORKFLOW` mà không ghi một phần. Dữ liệu gửi chứa hai bản sao của cùng một `BookId` trả về `400 DUPLICATE_BOOK_IN_REQUEST`.
...
- FR-FE07-014A: NẾU số bản sao thành viên đã yêu cầu trong ngày nghiệp vụ cộng với yêu cầu mới vượt quá hạn mức hằng ngày do FE04 xác định, hoặc số bản sao đã được phê duyệt trong ngày nghiệp vụ cộng với số bản sao sắp phê duyệt vượt quá hạn mức đó, thì hệ thống sẽ từ chối toàn bộ thao tác bằng `BORROW_DAILY_LIMIT_EXCEEDED`. Hạn mức là 5 với `APPROVED` và 3 với các trạng thái còn lại, tính theo ngày `Asia/Ho_Chi_Minh`.
...
- FR-FE07-020: NẾU chi tiết mượn đã quá hạn theo ngày nghiệp vụ `Asia/Ho_Chi_Minh` hiện tại, đã được gia hạn một lần, bị chặn do có khoản phạt chưa thanh toán hoặc có lượt đặt chỗ của thành viên khác, hệ thống sẽ từ chối gia hạn và giữ nguyên hạn trả hiện tại. Việc so sánh quá hạn phải dùng trình hỗ trợ ngày nghiệp vụ chung và không phụ thuộc múi giờ máy chủ. (Nguồn: BR-FE07-015, BR-FE07-018, AF-FE07-004, EC-FE07-010, AC-FE07-010)
- FR-FE07-021: NẾU thao tác trả hoặc gia hạn nhắm đến trạng thái chi tiết không hợp lệ, thao tác trả phát hiện bản sao vật lý không ở trạng thái `BORROWED`, hoặc ngày trả nằm trước `BorrowDate` hay sau ngày nghiệp vụ hiện tại trong `Asia/Ho_Chi_Minh`, hệ thống sẽ từ chối thao tác mà không thay đổi hạn trả, dữ liệu trả, trạng thái bản sao hoặc ghi kiểm toán thành công. Bản sao vật lý không nhất quán trả về `BORROW_STATE_CONFLICT`. (Nguồn: EC-FE07-008, EC-FE07-009, EC-FE07-010)
...
- FR-FE07-024: NẾU bản sao là `RESERVED` theo lượt đặt chỗ `NOTIFIED` do Thành viên mượn sở hữu, FE07 sẽ cho phép tạo yêu cầu và xác nhận lại quyền sở hữu đó trong quá trình phê duyệt.
...
- AC-FE07-003A: Với một `MEMBER` đang hoạt động nhưng chưa có trạng thái phê duyệt FE04 `APPROVED` đã yêu cầu 3 bản sao trong ngày nghiệp vụ hiện tại, khi yêu cầu thêm một bản sao thì FE07 trả về `BORROW_DAILY_LIMIT_EXCEEDED`; với trạng thái `APPROVED`, thành viên được yêu cầu tối đa 5 bản sao trong ngày nếu đáp ứng các quy tắc mượn còn lại.
...
- AC-FE07-006: Khi cả chi tiết và bản sao vật lý vẫn là `BORROWED`, nếu Thủ thư xử lý lượt trả bình thường, hệ thống sẽ lưu ngày nghiệp vụ trả sách theo `Asia/Ho_Chi_Minh` và đánh dấu bản sao là `AVAILABLE`; nếu có hàng đợi FE08 `ACTIVE`, quyền sở hữu hàng đợi vẫn được giữ và thao tác mượn thông thường tiếp tục bị chặn cho đến khi FE08 giải quyết. Nếu trạng thái bản sao không nhất quán, thao tác trả bị từ chối bằng `BORROW_STATE_CONFLICT` mà không thay đổi dữ liệu.
- AC-FE07-007: Với một bản sao được trả trong tình trạng hư hỏng, khi Thủ thư xử lý lượt trả thì hệ thống đánh dấu bản sao là `DAMAGED` và bản sao không còn sẵn sàng để mượn.
...
- AC-FE07-010: Với một bản sao đã mượn đang quá hạn theo ngày nghiệp vụ chung của `Asia/Ho_Chi_Minh`, đã được gia hạn, bị chặn do có khoản phạt chưa thanh toán hoặc có lượt đặt chỗ của thành viên khác, khi yêu cầu gia hạn được gửi từ máy chủ ở bất kỳ múi giờ nào thì hạn trả vẫn không đổi và hệ thống trả về lý do.
...
- AC-FE07-015: Với một lượt đặt chỗ đang hoạt động, khi thành viên khác tạo hoặc phê duyệt yêu cầu mượn thì FE07 trả về `409 RESERVATION_QUEUE_PRIORITY` và giữ nguyên toàn bộ trạng thái.
...
- AC-FE07-018: Với một bản sao có đầu sách tương ứng ở trạng thái `INACTIVE`, khi tạo hoặc phê duyệt yêu cầu mượn thì FE07 trả về `BOOK_INACTIVE` và giữ nguyên toàn bộ trạng thái.
...
- AC-FE07-020: Với ngày trả trước `BorrowDate` hoặc sau ngày nghiệp vụ `Asia/Ho_Chi_Minh` hiện tại, khi gửi yêu cầu trả thì FE07 trả về `INVALID_RETURN_DATE` và giữ nguyên toàn bộ trạng thái.
...
- AC-FE07-026: Với mảng vai trò tương thích cũ cố ý bị hỏng, chứa `MEMBER + LIBRARIAN` hoặc `MEMBER + ADMIN`, khi tác nhân trực tiếp mở hoặc gọi danh sách bản sao có thể yêu cầu mượn, thao tác tạo yêu cầu hoặc lịch sử cá nhân của Thành viên, giao diện sẽ chuyển hướng đến trang chủ nhân viên và backend trả về `403 ROLE_REQUIRED` mà không tạo hay tiết lộ trạng thái tự phục vụ của Thành viên; các tài khoản được lưu vẫn có chính xác một vai trò.
- AC-FE07-027: Khi FE08 liên kết bản sao được giữ cho Thành viên đến `/borrowing/new?bookId={bookId}&copyId={copyId}`, nếu danh sách FE07 chứa lượt giữ thuộc người yêu cầu, FE07 sẽ chọn trước chính xác bản sao và tạo yêu cầu `PENDING` thông thường để Thủ thư/Quản trị viên phê duyệt.
- AC-FE07-028: Cho Thành viên A có yêu cầu cấp bản sao đang chờ xử lý, khi Thành viên B tải danh sách hoặc gửi yêu cầu cho cùng bản sao đó, bản sao sẽ không xuất hiện trong danh sách và thao tác tạo trả về `409 COPY_PENDING_REQUEST_CONFLICT`; sau khi nhân viên từ chối yêu cầu của A, bản sao có thể được yêu cầu lại nếu đáp ứng mọi quy tắc FE07/FE08 khác.
...
- AC-FE07-030: Với Thành viên đã có yêu cầu đang chờ hoặc khoản mượn đang hoạt động cho một đầu sách, khi Thành viên tải danh sách hoặc gửi yêu cầu cho bản sao khác của đầu sách đó, đầu sách sẽ bị ẩn và thao tác tạo trả về `409 BOOK_ALREADY_IN_BORROWING_WORKFLOW`; sau khi yêu cầu bị từ chối hoặc bản sao được trả về trạng thái cuối, có thể tiếp tục tạo yêu cầu mới.
...
| EC-FE07-003 | `MEMBER` đang hoạt động không được FE04 phê duyệt | Không từ chối chỉ vì trạng thái hội viên; áp dụng hạn mức 3 bản sao mỗi ngày và tất cả quy tắc FE07 khác. |
...
| EC-FE07-019 | Việc gia hạn diễn ra theo múi giờ máy chủ khác với `Asia/Ho_Chi_Minh` | Dùng trình hỗ trợ ngày nghiệp vụ chung để kết quả kiểm tra điều kiện và hạn trả không phụ thuộc múi giờ máy chủ. |
...
| AuditLogs | Ghi lại các hoạt động mượn sách quan trọng. |
...
| returnDate | ngày | Cần thiết cho returned/lost/damaged | Mặc định là ngày nghiệp vụ `Asia/Ho_Chi_Minh` hiện tại; phải nằm trong khoảng từ `borrowDate` đến ngày nghiệp vụ hiện tại. |
...
| requestStatus | chuỗi | Có | Các giá trị: `PENDING`, `APPROVED`, `REJECTED`, `COMPLETED`, `CANCELLED`. Mỗi chi tiết trong lịch sử mượn hiển thị trạng thái của yêu cầu chứa chi tiết đó, tách biệt với `detailStatus` được lưu. |
...
| INV-FE07-A2 | Yêu cầu chỉ có thể chuyển sang `APPROVED` sau khi điều kiện của thành viên và khả năng mượn có xét trạng thái đặt chỗ được xác nhận lại (BR-FE07-008, BR-FE07-023). |
...
> `dueDate` trước ngày nghiệp vụ `Asia/Ho_Chi_Minh` hiện tại; FE09 sử dụng giá trị dẫn xuất này (BR-FE07-014).
...
| `BORROWED` | `BORROWED` | Gia hạn | Chưa quá hạn, không có khoản phạt chưa thanh toán, renewalCount = 0, không có xung đột đặt chỗ; hạn trả +14 ngày, renewalCount → 1 | MF-FE07-005, FR-FE07-009, BR-FE07-015, BR-FE07-018 |
...
| Bất kỳ lượt trả nào có ngày trước `BorrowDate` hoặc sau ngày nghiệp vụ `Asia/Ho_Chi_Minh` hiện tại | Từ chối chuyển trạng thái do ngày không hợp lệ (BR-FE07-011, FR-FE07-021, EC-FE07-009, EC-FE07-014). |
...
| INV-FE07-B5 | Việc chuyển sang `RETURNED`, `LOST` hoặc `DAMAGED` yêu cầu ngày trả được lưu trong khoảng bao gồm từ `BorrowDate` đến ngày nghiệp vụ `Asia/Ho_Chi_Minh` hiện tại (BR-FE07-011, FR-FE07-021). |
| INV-FE07-B6 | `BookCopies.Status` liên quan phải nhất quán với trạng thái chi tiết: `BORROWED` -> `BORROWED`, `RETURNED` -> `AVAILABLE`, `DAMAGED` -> `DAMAGED`, `LOST` -> `LOST`; lượt đặt chỗ FE08 `ACTIVE` có thể vẫn tồn tại trên bản sao `AVAILABLE` và tiếp tục chặn thao tác mượn thông thường; lượt giữ đã thông báo thuộc người yêu cầu trở thành `FULFILLED` khi phê duyệt (BR-FE07-007, BR-FE07-012, BR-FE07-013, BR-FE07-025, FR-FE07-012). |
...
| GET | `/api/borrow-requests/me` | Thành viên | Truy vấn: `status?, fromDate?, toDate?, page=1, limit=20` | Lịch sử mượn của chính mình được phân trang | `status` là trạng thái chi tiết, bao gồm `OVERDUE` được tính tại thời điểm đọc; khoảng ngày bao gồm cả hai đầu và dùng BorrowDate hoặc RequestDate cho các hàng còn ở trạng thái yêu cầu; thứ tự ổn định là BorrowDate DESC, giá trị null ở cuối, rồi BorrowDetailId DESC. Mỗi chi tiết trả về có `requestStatus` từ yêu cầu chứa nó; `status` vẫn là trạng thái chi tiết dùng cho bộ lọc. |
...
| PATCH | `/api/borrow-details/{borrowDetailId}/return` | Librarian/Admin | `{ condition: "NORMAL"|"DAMAGED"|"LOST", returnDate?: date, notes?: string }` | Chi tiết mượn đã cập nhật cùng `fineCandidate` | Mặc định là ngày nghiệp vụ `Asia/Ho_Chi_Minh` hiện tại; từ chối ngày tương lai hoặc trước ngày mượn; lượt trả bình thường giữ quyền sở hữu hàng đợi FE08 `ACTIVE` và ưu tiên mượn tương ứng. Phản hồi và kiểm toán dùng các giá trị hạn trả/ngày trả đã khóa trong giao dịch. |
| PATCH | `/api/borrow-details/{borrowDetailId}/renew` | Member/Librarian/Admin | Ghi chú tùy chọn | Hạn trả đã cập nhật | Mỗi tài khoản có một vai trò. Thủ thư/Quản trị viên được gia hạn cho nhiều Thành viên; Thành viên phải sở hữu chi tiết. Mọi quy tắc về điều kiện của chủ khoản mượn và ngày nghiệp vụ vẫn áp dụng. |
...
- NFR-FE07-SEC-003: Thành viên không được truy cập lịch sử mượn của thành viên khác.
...
- NFR-FE07-PERF-002: Truy vấn lịch sử mượn phải áp dụng bộ lọc thành viên, trạng thái, ngày tháng và phân trang trong cơ sở dữ liệu trước khi tạo danh sách kết quả; không được quét toàn bảng ở tầng ứng dụng.
...
- NFR-FE07-UX-001: Lỗi xác thực phải giải thích lý do: thành viên không hoạt động, giới hạn mượn, bản sao không có sẵn, ưu tiên hàng đợi đặt chỗ, xung đột trạng thái đặt chỗ, tiền phạt chưa thanh toán, khoản mượn quá hạn hoặc trạng thái không hợp lệ.
...
| Q-FE07-002 | Thời hạn cho mượn mặc định tính bằng ngày là bao nhiêu? | Team/Teacher | Đã giải quyết: 14 ngày theo lịch kể từ `BorrowDate` được lưu, là ngày nghiệp vụ đã phê duyệt trong `Asia/Ho_Chi_Minh` (DEC-GEN-002). |
...
| Q-FE07-009 | Hợp đồng truy vấn lịch sử mượn là gì? | Chuẩn hóa đặc tả 2026-07-17 | Đã giải quyết: `status?, fromDate?, toDate?, page?, limit?`; mặc định trang 1/giới hạn 20, tối đa 100, khoảng ngày nghiệp vụ bao gồm hai đầu, thứ tự BorrowDate/BorrowDetailId ổn định và xác thực trước truy vấn. |
...
- [x] Xác nhận siêu dữ liệu yêu cầu/phê duyệt/mượn bắt buộc và ngày nghiệp vụ `Asia/Ho_Chi_Minh`.
