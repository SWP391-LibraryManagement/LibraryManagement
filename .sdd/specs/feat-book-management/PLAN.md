...
Maintain the reconciled FE05 catalog against the approved v0.6.4 contract: deterministic public/staff queries, active reference-data reads, schema-aware deployment readiness, validated managed covers, atomic audited mutations, optimistic concurrency, explicit deactivate/reactivate commands, and read-only derived availability from FE06 copy state.
...
| Deployment readiness | `node --test tests/deployment/smokeStaging.test.js` | Staging smoke rejects a legacy metadata schema and accepts the canonical migrated schema. |
