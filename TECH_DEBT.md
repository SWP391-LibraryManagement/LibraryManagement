...
| TD-015 | FE11 User & Role | P1 | Service-level coverage exists for account setup/resend, transactional role outcomes, strict user list/detail behavior, and Audit Log authorization/pagination. Update/deactivation still need focused tests; Audit Logs still lack boundary-validation, search/filter/date, and redaction coverage. | VG FE11 P0-2; FE11 context drift audit | PARTIAL |
...
| TD-022 | FE11 User & Role | P1 | Admin role-action UI sends `roleName` in the request body/path, while the approved API and backend validators require positive numeric `roleId`; assignment/revocation from the UI cannot satisfy AC-FE11-013/014. | FE11 context drift audit | IN PROGRESS |
...
| TD-026 | FE11 Safe User Reads | P1 | `GET /api/users` emits an undocumented top-level `summary` in addition to the approved `data` + `pagination` envelope. The behavior entered outside `FE11-U01..U06` validation and lacks a focused repository contract assertion. | FE11 context drift audit; approved safe-read design | OPEN |
| TD-027 | FE11 Traceability | P2 | `SPEC.md` implementation-status cells still report completed account-setup, role, and safe-read requirements as `Ready for review`/`Not Started`. Reconcile evidence metadata in a separately approved SPEC maintenance change without altering requirements. | FE11 context drift audit | OPEN |
