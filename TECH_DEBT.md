...
| TD-026 | FE11 Safe User Reads | P1 | `GET /api/users` emits an undocumented top-level `summary` in addition to the approved `data` + `pagination` envelope. The behavior entered outside `FE11-U01..U06` validation and lacks a focused repository contract assertion. | FE11 context drift audit; approved safe-read design | IN PROGRESS |
| TD-027 | FE11 Traceability | P2 | `SPEC.md` implementation-status cells still report completed account-setup, role, and safe-read requirements as `Ready for review`/`Not Started`. Reconcile evidence metadata in a separately approved SPEC maintenance change without altering requirements. | FE11 context drift audit | IN PROGRESS |
